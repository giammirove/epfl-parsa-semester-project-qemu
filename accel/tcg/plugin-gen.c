...
static void gen_empty_udata_cb(void (*gen_helper)(TCGv_i32, TCGv_ptr))
{
  ...
  // TCGTemp *tmp = tcgv_i32_temp(cpu_index);
  // printf("gen %ld\n", tmp->val);
...
static void plugin_gen_empty_callback(enum plugin_gen_from from)
{
  ...
  case PLUGIN_GEN_FROM_TB:
    ...
    (void)gen_empty_udata_cb_no_wg;
    // backtrace_symbols_fd((void *)gen_helper_plugin_vcpu_udata_cb_no_rwg, 1,
    // 1);
    // TCGv_i32 cpu_index = tcg_temp_ebb_new_i32();
    // TCGv_ptr udata = tcg_temp_ebb_new_ptr();
    // gen_plugin_cb_start(from, PLUGIN_GEN_CB_UDATA, 0);
    // tcg_gen_movi_ptr(udata, 1);
    // tcg_gen_ld_i32(cpu_index, tcg_env,
    //                -offsetof(ArchCPU, env) + offsetof(CPUState, cpu_index));
    // gen_helper_plugin_vcpu_udata_cb_no_rwg(cpu_index, udata);
    // tcg_gen_plugin_cb_end();
 ...
 * we assert the ops we are replacing are the correct ones.
 */
static TCGOp *append_udata_cb_qflex(const struct qemu_plugin_dyn_cb *cb,
                                    TCGOp *begin_op, TCGOp *op, int *cb_idx)
{
  /* const_ptr */
  op = copy_const_ptr(&begin_op, op, cb->userp);
                                     ...
                                     TCGOp *begin_op)
{
  ...
  arr =
      g_array_sized_new(false, false, sizeof(struct qemu_plugin_dyn_cb), n_cbs);
...
static void inject_cb_qflex(const GArray *cbs, TCGOp *begin_op, op_ok_fn ok)
{
  TCGOp *end_op;
  TCGOp *op;
  int cb_idx = -1;
                                  ...
                                  TCGOp *begin_op, int insn_idx)
{
  ...
  // inject_udata_cb(insn->cbs[PLUGIN_CB_INSN][PLUGIN_CB_QFLEX], begin_op);
  inject_cb_qflex(insn->cbs[PLUGIN_CB_INSN][PLUGIN_CB_QFLEX], begin_op, op_ok);
}
