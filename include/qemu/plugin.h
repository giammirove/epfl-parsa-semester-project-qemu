...
static inline struct qemu_plugin_insn *qemu_plugin_insn_alloc(void)
{
  ...
  for (i = 0; i < PLUGIN_N_CB_TYPES; i++) {
    for (j = 0; j < PLUGIN_N_CB_SUBTYPES; j++) {
      insn->cbs[i][j] =
          g_array_new(false, false, sizeof(struct qemu_plugin_dyn_cb));
    }
  }
  return insn;
}
...
qemu_plugin_tb_insn_get(struct qemu_plugin_tb *tb, uint64_t pc)
{
...
static inline void qemu_plugin_qflex_state_hook(CPUState *cpu) {}
...
static inline void qemu_plugin_vcpu_syscall(CPUState *cpu, int64_t num,
...
static inline void qemu_plugin_vcpu_syscall_ret(CPUState *cpu, int64_t num,
                                                int64_t ret)
{
}
...
static inline void qemu_plugin_add_dyn_cb_arr(GArray *arr) {}
