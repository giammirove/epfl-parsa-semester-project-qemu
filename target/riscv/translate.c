...
static inline int get_olen(DisasContext *ctx) { return 16 << get_ol(ctx); }
...
static void gen_pc_plus_diff(TCGv target, DisasContext *ctx, target_long diff)
{
...
static int ex_plus_1(DisasContext *ctx, int nf) { return nf + 1; }
...
#define REQUIRE_64_OR_128BIT(ctx)                                              \
  do {                                                                         \
    if (get_xl(ctx) == MXL_RV32) {                                             \
      return false;                                                            \
    }                                                                          \
...
#define REQUIRE_EITHER_EXT(ctx, A, B)                                          \
  do {                                                                         \
    if (!ctx->cfg_ptr->ext_##A && !ctx->cfg_ptr->ext_##B) {                    \
      return false;                                                            \
    }                                                                          \
  } while (0)

static int ex_rvc_register(DisasContext *ctx, int reg) { return 8 + reg; }
...
static bool
gen_shift_imm_fn_per_ol(DisasContext *ctx, arg_shift *a, DisasExtend ext,
                        void (*f_tl)(TCGv, TCGv, target_long),
                        void (*f_32)(TCGv, TCGv, target_long),
                        void (*f_128)(TCGv, TCGv, TCGv, TCGv, target_long))
{
  int olen = get_olen(ctx);
  if (olen != TARGET_LONG_BITS) {
    if (olen == 32) {
...
static bool gen_unary_per_ol(DisasContext *ctx, arg_r2 *a, DisasExtend ext,
                             void (*f_tl)(TCGv, TCGv), void (*f_32)(TCGv, TCGv))
{
  int olen = get_olen(ctx);
...
void riscv_translate_init(void)
{
  ...
  for (i = 1; i < 32; i++) {
    cpu_gpr[i] = tcg_global_mem_new(tcg_env, offsetof(CPURISCVState, gpr[i]),
                                    riscv_int_regnames[i]);
    cpu_gprh[i] = tcg_global_mem_new(tcg_env, offsetof(CPURISCVState, gprh[i]),
                                     riscv_int_regnamesh[i]);
  }

  for (i = 0; i < 32; i++) {
    cpu_fpr[i] = tcg_global_mem_new_i64(
        tcg_env, offsetof(CPURISCVState, fpr[i]), riscv_fpr_regnames[i]);
  }
  ...
  cpu_vstart =
      tcg_global_mem_new(tcg_env, offsetof(CPURISCVState, vstart), "vstart");
