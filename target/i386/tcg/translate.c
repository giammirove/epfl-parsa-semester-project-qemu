...
static inline MemOp mo_b_d(int b, MemOp ot) { return b & 1 ? ot : MO_8; }
 ...
 * register's TCGv.
 */
static TCGv gen_op_deposit_reg_v(DisasContext *s, MemOp ot, int reg, TCGv dest,
                                 TCGv t0)
{
  switch (ot) {
  case MO_8:
...
static inline void gen_op_mov_v_reg(DisasContext *s, MemOp ot, TCGv t0, int reg)
{
...
static inline void gen_op_add_reg_im(DisasContext *s, MemOp size, int reg,
                                     int32_t val)
{
...
static inline void gen_op_add_reg(DisasContext *s, MemOp size, int reg,
                                  TCGv val)
{
...
static int cur_insn_len(DisasContext *s) { return s->pc - s->base.pc_next; }
...
static void gen_lea_v_seg(DisasContext *s, MemOp aflag, TCGv a0, int def_seg,
                          int ovr_seg)
{
...
static void gen_extu(MemOp ot, TCGv reg) { gen_ext_tl(reg, reg, ot, false); }

static void gen_exts(MemOp ot, TCGv reg) { gen_ext_tl(reg, reg, ot, true); }
                         ...
                         uint32_t svm_flags)
{
  ...
  if (GUEST(s)) {
    ...
    gen_helper_svm_check_io(tcg_env, port, tcg_constant_i32(svm_flags),
                            cur_insn_len_i32(s));
  }
  return true;
#endif
...
static CCPrepare gen_prepare_eflags_c(DisasContext *s, TCGv reg)
{
  ...
  add_sub:
    return (CCPrepare){.cond = TCG_COND_LTU,
                       .reg = t0,
                       .reg2 = t1,
                       .mask = -1,
                       .use_reg2 = true};
  ...
  case CC_OP_DECB ... CC_OP_DECQ:
    return (CCPrepare){
        .cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = -1, .no_setcond = true};
  ...
  case CC_OP_ADCOX:
    return (CCPrepare){
        .cond = TCG_COND_NE, .reg = cpu_cc_dst, .mask = -1, .no_setcond = true};

  case CC_OP_EFLAGS:
  case CC_OP_SARB ... CC_OP_SARQ:
    /* CC_SRC & 1 */
    return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_C};

  default:
    /* The need to compute only C from CC_OP_DYNAMIC is important
       in efficiently implementing e.g. INC at the start of a TB.  */
    gen_update_cc_op(s);
    gen_helper_cc_compute_c(reg, cpu_cc_dst, cpu_cc_src, cpu_cc_src2,
                            cpu_cc_op);
    return (CCPrepare){
        .cond = TCG_COND_NE, .reg = reg, .mask = -1, .no_setcond = true};
  }
}
...
static CCPrepare gen_prepare_eflags_p(DisasContext *s, TCGv reg)
{
  ...
  return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_P};
}
...
static CCPrepare gen_prepare_eflags_s(DisasContext *s, TCGv reg)
{
  ...
  case CC_OP_ADOX:
  case CC_OP_ADCOX:
    return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_S};
  case CC_OP_CLR:
  case CC_OP_POPCNT:
    return (CCPrepare){.cond = TCG_COND_NEVER, .mask = -1};
  default: {
...
static CCPrepare gen_prepare_eflags_o(DisasContext *s, TCGv reg)
{
  ...
  default:
    gen_compute_eflags(s);
    return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_O};
  }
}
...
static CCPrepare gen_prepare_eflags_z(DisasContext *s, TCGv reg)
{
  ...
  case CC_OP_ADOX:
  case CC_OP_ADCOX:
    return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_Z};
  case CC_OP_CLR:
    return (CCPrepare){.cond = TCG_COND_ALWAYS, .mask = -1};
  case CC_OP_POPCNT:
    return (CCPrepare){.cond = TCG_COND_EQ, .reg = cpu_cc_src, .mask = -1};
  default: {
    MemOp size = (s->cc_op - CC_OP_ADDB) & 3;
...
static CCPrepare gen_prepare_cc(DisasContext *s, int b, TCGv reg)
{
  ...
  case CC_OP_SUBB ... CC_OP_SUBQ:
    ...
    case JCC_BE:
      ...
      cc = (CCPrepare){.cond = TCG_COND_LEU,
                       .reg = s->tmp4,
                       .reg2 = t0,
                       .mask = -1,
                       .use_reg2 = true};
  ...
  slow_jcc:
    ...
    case JCC_BE:
      gen_compute_eflags(s);
      cc = (CCPrepare){
          .cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_Z | CC_C};
      break;
    case JCC_S:
    ...
    case JCC_L:
      ...
      cc = (CCPrepare){.cond = TCG_COND_NE, .reg = reg, .mask = CC_O};
      break;
    default:
    case JCC_LE:
      gen_compute_eflags(s);
      ...
      tcg_gen_addi_tl(reg, cpu_cc_src, CC_O - CC_S);
      cc = (CCPrepare){.cond = TCG_COND_NE, .reg = reg, .mask = CC_O | CC_Z};
      break;
    }
    break;
  }
...
static void gen_setcc1(DisasContext *s, int b, TCGv reg)
{
  ...
  if (cc.cond == TCG_COND_NE && !cc.use_reg2 && cc.imm == 0 && cc.mask != 0 &&
      (cc.mask & (cc.mask - 1)) == 0) {
...
static void gen_exception_gpf(DisasContext *s) { gen_exception(s, EXCP0D_GPF); }
...
static void gen_op(DisasContext *s1, int op, MemOp ot, int d)
{
  ...
  case OP_ADCL:
    ...
    if (s1->prefix & PREFIX_LOCK) {
      ...
      tcg_gen_atomic_add_fetch_tl(s1->T0, s1->A0, s1->T0, s1->mem_index,
                                  ot | MO_LE);
    }
    else {
  ...
  case OP_ADDL:
    if (s1->prefix & PREFIX_LOCK) {
      tcg_gen_atomic_add_fetch_tl(s1->T0, s1->A0, s1->T1, s1->mem_index,
                                  ot | MO_LE);
    }
    else {
      tcg_gen_add_tl(s1->T0, s1->T0, s1->T1);
  ...
  case OP_SUBL:
    if (s1->prefix & PREFIX_LOCK) {
      ...
      tcg_gen_atomic_fetch_add_tl(s1->cc_srcT, s1->A0, s1->T0, s1->mem_index,
                                  ot | MO_LE);
  ...
  case OP_ANDL:
    if (s1->prefix & PREFIX_LOCK) {
      tcg_gen_atomic_and_fetch_tl(s1->T0, s1->A0, s1->T1, s1->mem_index,
                                  ot | MO_LE);
    }
    else {
  ...
  case OP_ORL:
    if (s1->prefix & PREFIX_LOCK) {
      tcg_gen_atomic_or_fetch_tl(s1->T0, s1->A0, s1->T1, s1->mem_index,
                                 ot | MO_LE);
    }
    else {
  ...
  case OP_XORL:
    if (s1->prefix & PREFIX_LOCK) {
      tcg_gen_atomic_xor_fetch_tl(s1->T0, s1->A0, s1->T1, s1->mem_index,
                                  ot | MO_LE);
    }
    else {
...
static void gen_shift_flags(DisasContext *s, MemOp ot, TCGv result, TCGv shm1,
                            TCGv count, bool is_right)
{
  ...
  if (cc_op_live[s->cc_op] & USES_CC_DST) {
    tcg_gen_movcond_tl(TCG_COND_NE, cpu_cc_dst, count, z_tl, result,
                       cpu_cc_dst);
  }
  else {
  ...
  if (cc_op_live[s->cc_op] & USES_CC_SRC) {
    tcg_gen_movcond_tl(TCG_COND_NE, cpu_cc_src, count, z_tl, shm1, cpu_cc_src);
  }
  else {
...
static void gen_rot_rm_T1(DisasContext *s, MemOp ot, int op1, int is_right)
{
  ...
  tcg_gen_movcond_i32(TCG_COND_NE, cpu_cc_op, t1, t0, s->tmp2_i32, s->tmp3_i32);
...
static void gen_rotc_rm_T1(DisasContext *s, MemOp ot, int op1, int is_right)
{
  gen_compute_eflags(s);
...
static void gen_shiftd_rm_T1(DisasContext *s, MemOp ot, int op1, bool is_right,
                             TCGv count_in)
{
  ...
  default:
    ...
    tcg_gen_movcond_tl(TCG_COND_EQ, s->T1, count, s->tmp4, s->tmp4, s->T1);
...
static void gen_bnd_jmp(DisasContext *s)
{
  ...
  if ((s->prefix & PREFIX_REPNZ) == 0 && (s->flags & HF_MPX_EN_MASK) != 0 &&
...
static void do_gen_eob_worker(DisasContext *s, bool inhibit, bool recheck_tf,
                              bool jr)
{
  gen_update_cc_op(s);
...
static inline void gen_eob_worker(DisasContext *s, bool inhibit,
                                  bool recheck_tf)
{
...
static void gen_eob(DisasContext *s) { gen_eob_worker(s, false, false); }
...
{
  MemOp atom = (s->cpuid_ext_features & CPUID_EXT_AVX ? MO_ATOM_IFALIGN
...
static void gen_cmpxchg8b(DisasContext *s, CPUX86State *env, int modrm)
{
  ...
  if (s->prefix & PREFIX_LOCK) {
    ...
    tcg_gen_nonatomic_cmpxchg_i64(old, s->A0, cmp, val, s->mem_index, MO_TEUQ);
  }
  ...
  if (TARGET_LONG_BITS == 32) {
    ...
    tcg_gen_movcond_tl(TCG_COND_EQ, cpu_regs[R_EAX], Z, zero, s->T0,
                       cpu_regs[R_EAX]);
    tcg_gen_movcond_tl(TCG_COND_EQ, cpu_regs[R_EDX], Z, zero, s->T1,
                       cpu_regs[R_EDX]);
  }
...
next_byte:
  ...
  case 0xf7:
    ...
    case 2: /* not */
      if (s->prefix & PREFIX_LOCK) {
        ...
        tcg_gen_movi_tl(s->T0, ~0);
        tcg_gen_atomic_xor_fetch_tl(s->T0, s->A0, s->T0, s->mem_index,
                                    ot | MO_LE);
      }
      else {
    ...
    case 3: /* neg */
      if (s->prefix & PREFIX_LOCK) {
        ...
        tcg_gen_atomic_cmpxchg_tl(t0, a0, t0, t1, s->mem_index, ot | MO_LE);
    ...
    case 4: /* mul */
      ...
      case MO_32:
        ...
        tcg_gen_mulu2_i32(s->tmp2_i32, s->tmp3_i32, s->tmp2_i32, s->tmp3_i32);
...
#ifdef TARGET_X86_64
      case MO_64:
        tcg_gen_mulu2_i64(cpu_regs[R_EAX], cpu_regs[R_EDX], s->T0,
                          cpu_regs[R_EAX]);
        tcg_gen_mov_tl(cpu_cc_dst, cpu_regs[R_EAX]);
        tcg_gen_mov_tl(cpu_cc_src, cpu_regs[R_EDX]);
    ...
    case 5: /* imul */
      ...
      case MO_32:
        tcg_gen_trunc_tl_i32(s->tmp2_i32, s->T0);
        tcg_gen_trunc_tl_i32(s->tmp3_i32, cpu_regs[R_EAX]);
        tcg_gen_muls2_i32(s->tmp2_i32, s->tmp3_i32, s->tmp2_i32, s->tmp3_i32);
        tcg_gen_extu_i32_tl(cpu_regs[R_EAX], s->tmp2_i32);
        tcg_gen_extu_i32_tl(cpu_regs[R_EDX], s->tmp3_i32);
...
#ifdef TARGET_X86_64
      case MO_64:
        tcg_gen_muls2_i64(cpu_regs[R_EAX], cpu_regs[R_EDX], s->T0,
                          cpu_regs[R_EAX]);
        tcg_gen_mov_tl(cpu_cc_dst, cpu_regs[R_EAX]);
  ...
  case 0xff: /* GRP5 */
    ...
    do_lcall:
      if (PE(s) && !VM86(s)) {
        ...
        gen_helper_lcall_protected(tcg_env, s->tmp2_i32, s->T1,
                                   tcg_constant_i32(dflag - 1), eip_next_tl(s));
      }
      else {
        tcg_gen_trunc_tl_i32(s->tmp2_i32, s->T0);
        tcg_gen_trunc_tl_i32(s->tmp3_i32, s->T1);
        gen_helper_lcall_real(tcg_env, s->tmp2_i32, s->tmp3_i32,
                              tcg_constant_i32(dflag - 1), eip_next_i32(s));
      }
      s->base.is_jmp = DISAS_JUMP;
      break;
    case 4: /* jmp Ev */
    ...
    do_ljmp:
      if (PE(s) && !VM86(s)) {
        tcg_gen_trunc_tl_i32(s->tmp2_i32, s->T0);
        gen_helper_ljmp_protected(tcg_env, s->tmp2_i32, s->T1, eip_next_tl(s));
      }
      else {
  ...
  case 0x1c1: /* xadd Ev, Gv */
    ...
    if (mod == 3) {
      ...
      gen_lea_modrm(env, s, modrm);
      if (s->prefix & PREFIX_LOCK) {
        tcg_gen_atomic_fetch_add_tl(s->T1, s->A0, s->T0, s->mem_index,
                                    ot | MO_LE);
        tcg_gen_add_tl(s->T0, s->T0, s->T1);
      }
      else {
  ...
  case 0x1b1: /* cmpxchg Ev, Gv */
  {
    ...
    if (s->prefix & PREFIX_LOCK) {
      ...
      gen_lea_modrm(env, s, modrm);
      tcg_gen_atomic_cmpxchg_tl(oldv, s->A0, cmpv, newv, s->mem_index,
                                ot | MO_LE);
    }
    else {
      if (mod == 3) {
  ...
  case 0x1c7: /* cmpxchg8b */
    ...
    case 7: /* RDSEED, RDPID with f3 prefix */
      if (mod != 3 || (s->prefix & (PREFIX_LOCK | PREFIX_REPNZ))) {
        goto illegal_op;
      }
  ...
  case 0x87: /* xchg Ev, Gv */
    ...
    do_xchg_reg:
      ...
      tcg_gen_atomic_xchg_tl(s->T1, s->A0, s->T0, s->mem_index, ot | MO_LE);
      gen_op_mov_reg_v(s, ot, reg, s->T1);
    }
    break;
  case 0xc4: /* les Gv */
  ...
  case 0xd8 ... 0xdf: {
    ...
    if (mod != 3) {
      ...
      case 0x30 ... 0x37: /* fixxx */
      {
        ...
        case 3:
        default:
          tcg_gen_qemu_ld_i32(s->tmp2_i32, s->A0, s->mem_index, MO_LESW);
          gen_helper_fildl_FT0(tcg_env, s->tmp2_i32);
          break;
        }
      ...
      case 0x38 ... 0x3b: /* filds, fisttps, fists, fistps */
        ...
        case 1:
          ...
          default:
            ...
            tcg_gen_qemu_st_i32(s->tmp2_i32, s->A0, s->mem_index, MO_LEUW);
            break;
          }
          gen_helper_fpop(tcg_env);
          break;
      ...
      case 0x0c: /* fldenv mem */
        gen_helper_fldenv(tcg_env, s->A0, tcg_constant_i32(dflag - 1));
        update_fip = update_fdp = false;
        break;
      case 0x0d: /* fldcw mem */
        tcg_gen_qemu_ld_i32(s->tmp2_i32, s->A0, s->mem_index, MO_LEUW);
        gen_helper_fldcw(tcg_env, s->tmp2_i32);
        update_fip = update_fdp = false;
        break;
      case 0x0e: /* fnstenv mem */
        gen_helper_fstenv(tcg_env, s->A0, tcg_constant_i32(dflag - 1));
        update_fip = update_fdp = false;
        break;
      ...
      case 0x2c: /* frstor mem */
        gen_helper_frstor(tcg_env, s->A0, tcg_constant_i32(dflag - 1));
        update_fip = update_fdp = false;
        break;
      case 0x2e: /* fnsave mem */
        gen_helper_fsave(tcg_env, s->A0, tcg_constant_i32(dflag - 1));
        update_fip = update_fdp = false;
        break;
      ...
      if (update_fdp) {
        ...
        tcg_gen_st16_i32(s->tmp2_i32, tcg_env, offsetof(CPUX86State, fpds));
        tcg_gen_st_tl(last_addr, tcg_env, offsetof(CPUX86State, fpdp));
      }
    }
    else {
      /* register float ops */
      ...
      case 0x08: /* fld sti */
        ...
        gen_helper_fmov_ST0_STN(tcg_env, tcg_constant_i32((opreg + 1) & 7));
        break;
      case 0x09: /* fxchg sti */
    ...
    if (update_fip) {
      ...
      tcg_gen_st16_i32(s->tmp2_i32, tcg_env, offsetof(CPUX86State, fpcs));
      tcg_gen_st_tl(eip_cur_tl(s), tcg_env, offsetof(CPUX86State, fpip));
    }
  } break;
    /************************/
    /* string ops */
  ...
  case 0xe8: /* call im */
  {
    int diff = (dflag != MO_16 ? (int32_t)insn_get(env, s, MO_32)
  ...
  bt_op:
    ...
    if (s->prefix & PREFIX_LOCK) {
      ...
      case 0: /* bt */
        ...
        break;
      case 1: /* bts */
        tcg_gen_atomic_fetch_or_tl(s->T0, s->A0, s->tmp0, s->mem_index,
                                   ot | MO_LE);
        break;
      case 2: /* btr */
        tcg_gen_not_tl(s->tmp0, s->tmp0);
        tcg_gen_atomic_fetch_and_tl(s->T0, s->A0, s->tmp0, s->mem_index,
                                    ot | MO_LE);
        break;
      default:
      case 3: /* btc */
        tcg_gen_atomic_fetch_xor_tl(s->T0, s->A0, s->tmp0, s->mem_index,
                                    ot | MO_LE);
        break;
      }
    ...
    default:
      ...
      gen_compute_eflags(s);
      tcg_gen_deposit_tl(cpu_cc_src, cpu_cc_src, s->tmp4, ctz32(CC_C), 1);
      break;
    }
    break;
  ...
  case 0x1bd: /* bsr / lzcnt */
    ...
    if ((prefixes & PREFIX_REPZ) &&
        (b & 1 ? s->cpuid_ext3_features & CPUID_EXT3_ABM
  ...
  case 0x100:
    ...
    case 0: /* sldt */
      ...
      tcg_gen_ld32u_tl(s->T0, tcg_env, offsetof(CPUX86State, ldt.selector));
      ot = mod == 3 ? dflag : MO_16;
      gen_ldst_modrm(env, s, modrm, ot, OR_TMP0, 1);
      break;
    case 2: /* lldt */
    ...
    case 1: /* str */
      ...
      tcg_gen_ld32u_tl(s->T0, tcg_env, offsetof(CPUX86State, tr.selector));
      ot = mod == 3 ? dflag : MO_16;
      gen_ldst_modrm(env, s, modrm, ot, OR_TMP0, 1);
      break;
    case 3: /* ltr */
  ...
  case 0x101:
    ...
    switch (modrm) {
      ...
      gen_lea_modrm(env, s, modrm);
      tcg_gen_ld32u_tl(s->T0, tcg_env, offsetof(CPUX86State, gdt.limit));
    ...
    case 0xca: /* clac */
      if (!(s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_SMAP) || CPL(s) != 0) {
        goto illegal_op;
      }
    ...
    case 0xd1: /* xsetbv */
      ...
      tcg_gen_concat_tl_i64(s->tmp1_i64, cpu_regs[R_EAX], cpu_regs[R_EDX]);
      tcg_gen_trunc_tl_i32(s->tmp2_i32, cpu_regs[R_ECX]);
    ...
    case 0xf8: /* swapgs */
      ...
      if (CODE64(s)) {
        if (check_cpl0(s)) {
          ...
          tcg_gen_st_tl(s->T0, tcg_env, offsetof(CPUX86State, kernelgsbase));
        }
        break;
      }
#endif
    ...
    if (s->flags & HF_MPX_EN_MASK) {
      ...
      if (prefixes & PREFIX_REPZ) {
        ...
        if (mod == 3) {
          ...
          gen_lea_modrm(env, s, modrm);
          if (CODE64(s)) {
            tcg_gen_qemu_ld_i64(cpu_bndl[reg], s->A0, s->mem_index, MO_LEUQ);
            tcg_gen_addi_tl(s->A0, s->A0, 8);
            tcg_gen_qemu_ld_i64(cpu_bndu[reg], s->A0, s->mem_index, MO_LEUQ);
          }
          else {
            tcg_gen_qemu_ld_i64(cpu_bndl[reg], s->A0, s->mem_index, MO_LEUL);
            tcg_gen_addi_tl(s->A0, s->A0, 4);
            tcg_gen_qemu_ld_i64(cpu_bndu[reg], s->A0, s->mem_index, MO_LEUL);
          }
  ...
  case 0x11b:
    ...
    if (s->flags & HF_MPX_EN_MASK) {
      ...
      if (mod != 3 && (prefixes & PREFIX_REPZ)) {
        ...
        if (mod == 3) {
          ...
          gen_lea_modrm(env, s, modrm);
          if (CODE64(s)) {
            tcg_gen_qemu_st_i64(cpu_bndl[reg], s->A0, s->mem_index, MO_LEUQ);
            tcg_gen_addi_tl(s->A0, s->A0, 8);
            tcg_gen_qemu_st_i64(cpu_bndu[reg], s->A0, s->mem_index, MO_LEUQ);
          }
          else {
            tcg_gen_qemu_st_i64(cpu_bndl[reg], s->A0, s->mem_index, MO_LEUL);
            tcg_gen_addi_tl(s->A0, s->A0, 4);
            tcg_gen_qemu_st_i64(cpu_bndu[reg], s->A0, s->mem_index, MO_LEUL);
          }
        }
      }
      else if (mod != 3) {
        ...
        if (CODE64(s)) {
          gen_helper_bndstx64(tcg_env, s->A0, s->T0, cpu_bndl[reg],
                              cpu_bndu[reg]);
        }
        else {
          gen_helper_bndstx32(tcg_env, s->A0, s->T0, cpu_bndl[reg],
                              cpu_bndu[reg]);
        }
      }
    }
    ...
    break;
  case 0x119:
  case 0x11c ... 0x11f: /* nop (multi byte) */
    modrm = x86_ldub_code(env, s);
    gen_nop_modrm(env, s, modrm);
    break;
  ...
  case 0x1ae:
    ...
    switch (modrm) {
      CASE_MODRM_MEM_OP(0)
          : /* fxsave */
            if (!(s->cpuid_features & CPUID_FXSR) || (prefixes & PREFIX_LOCK))
      {
        goto illegal_op;
      }
    ...
    case 0xd8 ... 0xdf: /* wrgsbase (f3 0f ae /3) */
      if (CODE64(s) && (prefixes & PREFIX_REPZ) && !(prefixes & PREFIX_LOCK) &&
    ...
    case 0xf9 ... 0xff: /* sfence */
      if (!(s->cpuid_features & CPUID_SSE) || (prefixes & PREFIX_LOCK)) {
        goto illegal_op;
      }
    ...
    case 0xf0 ... 0xf7: /* mfence */
      if (!(s->cpuid_features & CPUID_SSE2) || (prefixes & PREFIX_LOCK)) {
        goto illegal_op;
      }
...
void tcg_x86_init(void)
{
...
#ifdef TARGET_X86_64
      [R_EAX] = "rax", [R_EBX] = "rbx", [R_ECX] = "rcx", [R_EDX] = "rdx",
      [R_ESI] = "rsi", [R_EDI] = "rdi", [R_EBP] = "rbp", [R_ESP] = "rsp",
      [8] = "r8",      [9] = "r9",      [10] = "r10",    [11] = "r11",
      [12] = "r12",    [13] = "r13",    [14] = "r14",    [15] = "r15",
#else
      [R_EAX] = "eax", [R_EBX] = "ebx", [R_ECX] = "ecx", [R_EDX] = "edx",
      [R_ESI] = "esi", [R_EDI] = "edi", [R_EBP] = "ebp", [R_ESP] = "esp",
#endif
  };
  ...
  static const char seg_base_names[6][8] = {
      [R_CS] = "cs_base", [R_DS] = "ds_base", [R_ES] = "es_base",
      [R_FS] = "fs_base", [R_GS] = "gs_base", [R_SS] = "ss_base",
  };
  static const char bnd_regl_names[4][8] = {"bnd0_lb", "bnd1_lb", "bnd2_lb",
                                            "bnd3_lb"};
  static const char bnd_regu_names[4][8] = {"bnd0_ub", "bnd1_ub", "bnd2_ub",
                                            "bnd3_ub"};
  int i;

  cpu_cc_op =
      tcg_global_mem_new_i32(tcg_env, offsetof(CPUX86State, cc_op), "cc_op");
  cpu_cc_dst =
      tcg_global_mem_new(tcg_env, offsetof(CPUX86State, cc_dst), "cc_dst");
  cpu_cc_src =
      tcg_global_mem_new(tcg_env, offsetof(CPUX86State, cc_src), "cc_src");
  cpu_cc_src2 =
      tcg_global_mem_new(tcg_env, offsetof(CPUX86State, cc_src2), "cc_src2");
  cpu_eip = tcg_global_mem_new(tcg_env, offsetof(CPUX86State, eip), eip_name);

  for (i = 0; i < CPU_NB_REGS; ++i) {
    cpu_regs[i] = tcg_global_mem_new(tcg_env, offsetof(CPUX86State, regs[i]),
                                     reg_names[i]);
  }

  for (i = 0; i < 6; ++i) {
    cpu_seg_base[i] = tcg_global_mem_new(
        tcg_env, offsetof(CPUX86State, segs[i].base), seg_base_names[i]);
  }

  for (i = 0; i < 4; ++i) {
    cpu_bndl[i] = tcg_global_mem_new_i64(
        tcg_env, offsetof(CPUX86State, bnd_regs[i].lb), bnd_regl_names[i]);
    cpu_bndu[i] = tcg_global_mem_new_i64(
        tcg_env, offsetof(CPUX86State, bnd_regs[i].ub), bnd_regu_names[i]);
  }
}
