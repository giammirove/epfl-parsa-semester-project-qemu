...
void a64_translate_init(void)
{
  ...
  for (i = 0; i < 32; i++) {
    cpu_X[i] = tcg_global_mem_new_i64(tcg_env, offsetof(CPUARMState, xregs[i]),
                                      regnames[i]);
  }

  cpu_exclusive_high = tcg_global_mem_new_i64(
      tcg_env, offsetof(CPUARMState, exclusive_high), "exclusive_high");
}
...
{
  tcg_gen_st_i32(tcg_constant_i32(val), tcg_env, offsetof(CPUARMState, btype));
}
...
static void gen_probe_access(DisasContext *s, TCGv_i64 ptr, MMUAccessType acc,
                             int log2_size)
{
  gen_helper_probe_access(tcg_env, ptr, tcg_constant_i32(acc),
...
{
  return gen_mte_check1_mmuidx(s, addr, is_write, tag_checked, memop, false,
                               get_mem_index(s));
}
 ...
 */
static void check_lse2_align(DisasContext *s, int rn, int imm, bool is_write,
                             MemOp mop)
{
  TCGv_i32 tmp;
  TCGv_i64 addr;
...
static MemOp check_atomic_align(DisasContext *s, int rn, MemOp mop)
{
   ...
   */
  if (size == MO_128) {
    return finalize_memop_atom(s, MO_128 | MO_ALIGN, MO_ATOM_IFALIGN_PAIR);
  }
...
{
  gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn), imm,
          is_q ? 16 : 8, vec_full_reg_size(s));
}
...
{
  gvec_fn(vece, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
          vec_full_reg_offset(s, rm), vec_full_reg_offset(s, rx), is_q ? 16 : 8,
          vec_full_reg_size(s));
}
...
{
  tcg_gen_gvec_2_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
...
{
  tcg_gen_gvec_3_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                              ...
                              gen_helper_gvec_3_ptr *fn)
{
  ...
  tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                     vec_full_reg_offset(s, rm), fpst, is_q ? 16 : 8,
                     vec_full_reg_size(s), data, fn);
}

/* Expand a 3-operand + qc + operation using an out-of-line helper.  */
static void gen_gvec_op3_qc(DisasContext *s, bool is_q, int rd, int rn, int rm,
                            gen_helper_gvec_3_ptr *fn)
{
  ...
  tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                     vec_full_reg_offset(s, rm), qc_ptr, is_q ? 16 : 8,
                     vec_full_reg_size(s), 0, fn);
}

/* Expand a 4-operand operation using an out-of-line helper.  */
static void gen_gvec_op4_ool(DisasContext *s, bool is_q, int rd, int rn, int rm,
                             int ra, int data, gen_helper_gvec_4 *fn)
{
  tcg_gen_gvec_4_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                     vec_full_reg_offset(s, rm), vec_full_reg_offset(s, ra),
                     is_q ? 16 : 8, vec_full_reg_size(s), data, fn);
}
...
{
  TCGv_ptr fpst = fpstatus_ptr(is_fp16 ? FPST_FPCR_F16 : FPST_FPCR);
  tcg_gen_gvec_4_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                     vec_full_reg_offset(s, rm), vec_full_reg_offset(s, ra),
                             ...
                             bool iss_ar)
{
  ...
  if (iss_valid) {
    ...
    syn = syn_data_abort_with_iss(0, (memop & MO_SIZE), false, iss_srt, iss_sf,
...
static void do_gpr_st(DisasContext *s, TCGv_i64 source, TCGv_i64 tcg_addr,
                      MemOp memop, bool iss_valid, unsigned int iss_srt,
                      bool iss_sf, bool iss_ar)
{
  do_gpr_st_memidx(s, source, tcg_addr, memop, get_mem_index(s), iss_valid,
                             ...
                             bool iss_ar)
{
  ...
  if (iss_valid) {
    ...
    syn =
        syn_data_abort_with_iss(0, (memop & MO_SIZE), (memop & MO_SIGN) != 0,
                                iss_srt, iss_sf, iss_ar, 0, 0, 0, 0, 0, false);
    disas_set_insn_syndrome(s, syn);
  }
}

static void do_gpr_ld(DisasContext *s, TCGv_i64 dest, TCGv_i64 tcg_addr,
                      MemOp memop, bool extend, bool iss_valid,
...
static bool fp_access_check_only(DisasContext *s)
{
  if (s->fp_excp_el) {
    ...
    gen_exception_insn_el(s, 0, EXCP_UDEF, syn_fp_access_trap(1, 0xe, false, 0),
                          s->fp_excp_el);
    return false;
  }
  s->fp_access_checked = true;
...
bool sve_access_check(DisasContext *s)
{
  if (s->pstate_sm || !dc_isar_feature(aa64_sve, s)) {
    ...
    gen_exception_insn_el(s, 0, EXCP_UDEF, syn_sve_access_trap(),
                          s->sve_excp_el);
    goto fail_exit;
  }
 ...
 */
static void ext_and_shift_reg(TCGv_i64 tcg_out, TCGv_i64 tcg_in, int option,
                              unsigned int shift)
{
...
static bool trans_CBZ(DisasContext *s, arg_cbz *a)
{
  ...
  tcg_gen_brcondi_i64(a->nz ? TCG_COND_NE : TCG_COND_EQ, tcg_cmp, 0,
                      match.label);
  gen_goto_tb(s, 0, 4);
...
static void gen_sysreg_undef(DisasContext *s, bool isread, uint8_t op0,
                             uint8_t op1, uint8_t op2, uint8_t crn, uint8_t crm,
                             uint8_t rt)
{
  /*
                       ...
                       unsigned int crm, unsigned int rt)
{
  ...
  if (ri->accessfn || (ri->fgt && s->fgt_active)) {
    ...
    gen_helper_access_check_cp_reg(tcg_ri, tcg_env, tcg_constant_i32(key),
  ...
  case ARM_CP_DC_ZVA:
    ...
    if (s->mte_active[0]) {
      ...
      gen_helper_mte_check_zva(tcg_rt, tcg_env, tcg_constant_i32(desc),
                               cpu_reg(s, rt));
    }
    else {
...
static bool trans_STLR(DisasContext *s, arg_stlr *a)
{
  ...
  clean_addr =
      gen_mte_check1(s, cpu_reg_sp(s, a->rn), true, a->rn != 31, memop);
  do_gpr_st(s, cpu_reg(s, a->rt), clean_addr, memop, true, a->rt, iss_sf,
            a->lasr);
  return true;
}
...
static bool trans_LDAR(DisasContext *s, arg_stlr *a)
{
  ...
  clean_addr =
      gen_mte_check1(s, cpu_reg_sp(s, a->rn), false, a->rn != 31, memop);
  do_gpr_ld(s, cpu_reg(s, a->rt), clean_addr, memop, false, true, a->rt, iss_sf,
            a->lasr);
  tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
  return true;
}
                                 ...
                                 uint64_t offset, bool is_store, MemOp mop)
{
  ...
  *clean_addr = gen_mte_checkN(s, *dirty_addr, is_store, (a->w || a->rn != 31),
                               2 << a->sz, mop);
}
                                 ...
                                 uint64_t offset, bool is_store, MemOp mop)
{
  ...
  *clean_addr = gen_mte_check1_mmuidx(
      s, *dirty_addr, is_store, a->w || a->rn != 31, mop, a->unpriv, memidx);
}
...
static bool trans_STR_i(DisasContext *s, arg_ldst_imm *a)
{
  ...
  do_gpr_st_memidx(s, tcg_rt, clean_addr, mop, memidx, iss_valid, a->rt, iss_sf,
                   false);
...
static bool trans_LDR_i(DisasContext *s, arg_ldst_imm *a)
{
  ...
  do_gpr_ld_memidx(s, tcg_rt, clean_addr, mop, a->ext, memidx, iss_valid, a->rt,
                   iss_sf, false);
  op_addr_ldst_imm_post(s, a, dirty_addr, a->imm);
  return true;
}
...
static void op_addr_ldst_pre(DisasContext *s, arg_ldst *a, TCGv_i64 *clean_addr,
                             TCGv_i64 *dirty_addr, bool is_store, MemOp memop)
{
  TCGv_i64 tcg_rm;
...
TRANS_FEAT(LDADD, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_add_i64,
           0, false)
TRANS_FEAT(LDCLR, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_and_i64,
           0, true)
TRANS_FEAT(LDEOR, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_xor_i64,
           0, false)
TRANS_FEAT(LDSET, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_or_i64, 0,
           false)
TRANS_FEAT(LDSMAX, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_smax_i64,
           MO_SIGN, false)
TRANS_FEAT(LDSMIN, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_smin_i64,
           MO_SIGN, false)
TRANS_FEAT(LDUMAX, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_umax_i64,
           0, false)
TRANS_FEAT(LDUMIN, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_fetch_umin_i64,
           0, false)
TRANS_FEAT(SWP, aa64_atomics, do_atomic_ld, a, tcg_gen_atomic_xchg_i64, 0,
           false)
...
static bool trans_LDAPR(DisasContext *s, arg_LDAPR *a)
{
  ...
  if (!dc_isar_feature(aa64_atomics, s) || !dc_isar_feature(aa64_rcpc_8_3, s)) {
    return false;
  }
  if (a->rn == 31) {
    gen_check_sp_alignment(s);
   ...
   */
  do_gpr_ld(s, cpu_reg(s, a->rt), clean_addr, mop, false, true, a->rt, iss_sf,
            true);
  tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
  return true;
}
...
static bool trans_LDAPR_i(DisasContext *s, arg_ldapr_stlr_i *a)
{
   ...
   */
  do_gpr_ld(s, cpu_reg(s, a->rt), clean_addr, mop, a->ext, true, a->rt, iss_sf,
            true);
  tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
  return true;
}
...
static bool trans_LD_single(DisasContext *s, arg_ldst_single *a)
{
  ...
  mop = finalize_memop_asimd(s, a->scale);
  clean_addr =
      gen_mte_checkN(s, tcg_rn, false, a->p || a->rn != 31, total, mop);
...
static bool trans_LD_single_repl(DisasContext *s, arg_LD_single_repl *a)
{
  ...
  for (xs = 0, rt = a->rt; xs < a->selem; xs++, rt = (rt + 1) % 32) {
    ...
    tcg_gen_gvec_dup_i64(a->scale, vec_full_reg_offset(s, rt), (a->q + 1) * 8,
                         vec_full_reg_size(s), tcg_tmp);
    tcg_gen_add_i64(clean_addr, clean_addr, tcg_ebytes);
  }
...
static bool do_SET(DisasContext *s, arg_set *a, bool is_epilogue, bool is_setg,
                   SetFn fn)
{
  int memidx;
   ...
   */
  if (a->rs == a->rn || a->rs == a->rd || a->rn == a->rd || a->rd == 31 ||
      a->rn == 31) {
    return false;
  }
   ...
   */
  syndrome = syn_mop(true, is_setg, (a->nontemp << 1) | a->unpriv, is_epilogue,
...
static bool do_CPY(DisasContext *s, arg_cpy *a, bool is_epilogue, CpyFn fn)
{
   ...
   * we pass wrong_option == false: helper function may set that bit.
   */
  syndrome = syn_mop(false, false, a->options, is_epilogue, false, true, a->rd,
                     a->rs, a->rn);
...
static bool gen_rri(DisasContext *s, arg_rri_sf *a, bool rd_sp, bool rn_sp,
                    ArithTwoOp *fn)
{
                                      ...
                                      bool sub_op)
{
  ...
  if (s->ata[0]) {
    gen_helper_addsubg(tcg_rd, tcg_env, tcg_rn, tcg_constant_i32(imm),
...
static void disas_data_proc_2src(DisasContext *s, uint32_t insn)
{
  ...
  case 4: /* IRG */
    ...
    if (s->ata[0]) {
      gen_helper_irg(cpu_reg_sp(s, rd), tcg_env, cpu_reg_sp(s, rn),
                     cpu_reg(s, rm));
    }
    else {
      gen_address_with_allocation_tag0(cpu_reg_sp(s, rd), cpu_reg_sp(s, rn));
    }
    break;
  case 5: /* GMI */
    if (sf == 0 || !dc_isar_feature(aa64_mte_insn_reg, s)) {
  ...
  case 12: /* PACGA */
    ...
    gen_helper_pacga(cpu_reg(s, rd), tcg_env, cpu_reg(s, rn),
                     cpu_reg_sp(s, rm));
    break;
  case 16:
  case 17:
...
static void disas_fp_csel(DisasContext *s, uint32_t insn)
{
  ...
  a64_test_cc(&c, cond);
  tcg_gen_movcond_i64(c.cond, t_true, c.value, tcg_constant_i64(0), t_true,
                      t_false);
                           ...
                           bool itof, int rmode, int scale, int sf, int type)
{
  ...
  if (itof) {
    ...
    case 3: /* float16 */
      ...
      if (sf) {
        if (is_signed) {
          gen_helper_vfp_tosqh(tcg_int, tcg_single, tcg_shift, tcg_fpstatus);
        }
        else {
          gen_helper_vfp_touqh(tcg_int, tcg_single, tcg_shift, tcg_fpstatus);
        }
      }
      else {
        TCGv_i32 tcg_dest = tcg_temp_new_i32();
        if (is_signed) {
          gen_helper_vfp_toslh(tcg_dest, tcg_single, tcg_shift, tcg_fpstatus);
        }
        else {
          gen_helper_vfp_toulh(tcg_dest, tcg_single, tcg_shift, tcg_fpstatus);
        }
        tcg_gen_extu_i32_i64(tcg_int, tcg_dest);
      }
      break;
...
static void disas_simd_tb(DisasContext *s, uint32_t insn)
{
  ...
  tcg_gen_gvec_2_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rm),
                     tcg_env, is_q ? 16 : 8, vec_full_reg_size(s),
                     (len << 6) | (is_tbx << 5) | rn, gen_helper_simd_tblx);
}
  ...
  for (i = 0; i < elements; i++) {
    ...
    case 1: /* UZP1/2 */
    {
      ...
      if (i < midpoint) {
        ...
        read_vec_element(s, tcg_ele, rm, 2 * (i - midpoint) + part, size);
      }
      break;
    }
    case 2: /* TRN1/2 */
...
static void disas_simd_across_lanes(DisasContext *s, uint32_t insn)
{
  ...
  if (!is_fp) {
    ...
    TCGv_i32 tcg_res32 =
        do_reduction_op(s, fpopcode, rn, esize, (is_q ? 128 : 64), vmap, fpst);
  ...
  if (round) {
    if (extended_result) {
      ...
      if (!is_u) {
        ...
        tcg_gen_add2_i64(tcg_src, tcg_src_hi, tcg_src, tcg_src_hi, tcg_rnd,
                         tcg_zero);
      }
      else {
        tcg_gen_add2_i64(tcg_src, tcg_src_hi, tcg_src, tcg_zero, tcg_rnd,
                         tcg_zero);
      }
    }
    else {
                                    ...
                                    int immb, int opcode, int rn, int rd)
{
  ...
  if (insert) {
    ...
    handle_shri_with_rndacc(tcg_rd, tcg_rn, tcg_round, accumulate, is_u, size,
                            shift);
  }
                                   ...
                                   int immb, int opcode, int rn, int rd)
{
  ...
  for (i = 0; i < elements; i++) {
    ...
    handle_shri_with_rndacc(tcg_rd, tcg_rn, tcg_round, false, is_u_shift,
                            size + 1, shift);
                                   ...
                                   int size)
{
  ...
  if (size == MO_64) {
    ...
    for (pass = 0; pass < elements; pass++) {
      ...
      if (is_signed) {
        gen_helper_vfp_sqtod(tcg_double, tcg_int64, tcg_shift, tcg_fpst);
      }
      else {
        gen_helper_vfp_uqtod(tcg_double, tcg_int64, tcg_shift, tcg_fpst);
      }
      if (elements == 1) {
        write_fp_dreg(s, rd, tcg_double);
      }
    ...
    for (pass = 0; pass < elements; pass++) {
      ...
      case MO_32:
        if (fracbits) {
          if (is_signed) {
            gen_helper_vfp_sltos(tcg_float, tcg_int32, tcg_shift, tcg_fpst);
          }
          else {
            gen_helper_vfp_ultos(tcg_float, tcg_int32, tcg_shift, tcg_fpst);
          }
        }
        else {
          if (is_signed) {
      ...
      case MO_16:
        if (fracbits) {
          if (is_signed) {
            gen_helper_vfp_sltoh(tcg_float, tcg_int32, tcg_shift, tcg_fpst);
          }
          else {
            gen_helper_vfp_ultoh(tcg_float, tcg_int32, tcg_shift, tcg_fpst);
          }
        }
        else {
          if (is_signed) {
...
static void disas_simd_scalar_shift_imm(DisasContext *s, uint32_t insn)
{
  ...
  case 0x1c: /* SCVTF, UCVTF */
    handle_simd_shift_intfp_conv(s, true, false, is_u, immh, immb, opcode, rn,
                                 rd);
    break;
  ...
  case 0x11: /* SQRSHRUN, SQRSHRUN2 */
    ...
    handle_vec_simd_sqshrn(s, true, false, false, true, immh, immb, opcode, rn,
                           rd);
    break;
  case 0x12: /* SQSHRN, SQSHRN2, UQSHRN */
  case 0x13: /* SQRSHRN, SQRSHRN2, UQRSHRN, UQRSHRN2 */
    handle_vec_simd_sqshrn(s, true, false, is_u, is_u, immh, immb, opcode, rn,
                           rd);
    break;
  case 0xc: /* SQSHLU */
    if (!is_u) {
...
static void disas_simd_scalar_three_reg_diff(DisasContext *s, uint32_t insn)
{
  ...
  if (size == 2) {
    ...
    case 0x9: /* SQDMLAL, SQDMLAL2 */
      ...
      gen_helper_neon_addl_saturate_s64(tcg_res, tcg_env, tcg_res, tcg_op1);
      break;
    default:
      g_assert_not_reached();
    }
    ...
    case 0x9: /* SQDMLAL, SQDMLAL2 */
    {
      ...
      gen_helper_neon_addl_saturate_s32(tcg_res, tcg_env, tcg_res, tcg_op3);
      break;
    }
    default:
      g_assert_not_reached();
  ...
  for (pass = 0; pass < elements; pass++) {
    if (size) {
      ...
      case 0x19: /* FMLA */
        ...
        gen_helper_vfp_muladdd(tcg_res, tcg_op1, tcg_op2, tcg_res, fpst);
        break;
      case 0x18: /* FMAXNM */
        gen_helper_vfp_maxnumd(tcg_res, tcg_op1, tcg_op2, fpst);
        break;
      ...
      case 0x19: /* FMLA */
        ...
        gen_helper_vfp_muladds(tcg_res, tcg_op1, tcg_op2, tcg_res, fpst);
        break;
      case 0x1a: /* FADD */
        gen_helper_vfp_adds(tcg_res, tcg_op1, tcg_op2, fpst);
        break;
  ...
  for (pass = 0; pass < passes; pass++) {
    ...
    case 0x14: /* SQXTN, UQXTN */
    {
      static NeonGenNarrowEnvFn *const fns[3][2] = {
          ...
          {gen_helper_neon_narrow_sat_s16, gen_helper_neon_narrow_sat_u16},
          {gen_helper_neon_narrow_sat_s32, gen_helper_neon_narrow_sat_u32},
      };
      genenvfn = fns[size][u];
      break;
    }
...
static void disas_simd_scalar_two_reg_misc(DisasContext *s, uint32_t insn)
{
  ...
  if (size == 3) {
    ...
    case 0x3a: /* FCVTPS */
    case 0x3b: /* FCVTZS */
      gen_helper_vfp_tosls(tcg_rd, tcg_rn, tcg_constant_i32(0), tcg_fpstatus);
      break;
    case 0x5a: /* FCVTNU */
    case 0x5b: /* FCVTMU */
    case 0x5c: /* FCVTAU */
    case 0x7a: /* FCVTPU */
    case 0x7b: /* FCVTZU */
      gen_helper_vfp_touls(tcg_rd, tcg_rn, tcg_constant_i32(0), tcg_fpstatus);
      break;
    default:
      g_assert_not_reached();
    }
  ...
  case 0x00: /* SSHR / USHR */
    if (is_u) {
      if (shift == 8 << size) {
        ...
        tcg_gen_gvec_dup_imm(size, vec_full_reg_offset(s, rd), is_q ? 16 : 8,
                             vec_full_reg_size(s), 0);
        return;
      }
                                 ...
                                 int opcode, int rn, int rd)
{
  ...
  for (i = 0; i < elements; i++) {
    ...
    handle_shri_with_rndacc(tcg_rd, tcg_rn, tcg_round, false, true, size + 1,
                            shift);
...
static void disas_simd_shift_imm(DisasContext *s, uint32_t insn)
{
  ...
  case 0x11: /* RSHRN / SQRSHRUN */
    if (is_u) {
      handle_vec_simd_sqshrn(s, false, is_q, false, true, immh, immb, opcode,
                             rn, rd);
    }
    else {
  ...
  case 0x13: /* SQRSHRN / UQRSHRN */
    handle_vec_simd_sqshrn(s, false, is_q, is_u, is_u, immh, immb, opcode, rn,
                           rd);
    break;
  case 0x14: /* SSHLL / USHLL */
    ...
    break;
  case 0x1c: /* SCVTF / UCVTF */
    handle_simd_shift_intfp_conv(s, false, is_q, is_u, immh, immb, opcode, rn,
                                 rd);
    break;
  case 0xc: /* SQSHLU */
    if (!is_u) {
  ...
  if (size == 2) {
    for (pass = 0; pass < 2; pass++) {
      ...
      case 7: /* SABDL, SABDL2, UABDL, UABDL2 */
      {
        ...
        tcg_gen_movcond_i64(is_u ? TCG_COND_GEU : TCG_COND_GE, tcg_passres,
      ...
      case 13: /* SQDMULL, SQDMULL2 */
        tcg_gen_mul_i64(tcg_passres, tcg_op1, tcg_op2);
        gen_helper_neon_addl_saturate_s64(tcg_passres, tcg_env, tcg_passres,
                                          tcg_passres);
        break;
      default:
        g_assert_not_reached();
      ...
      if (opcode == 9 || opcode == 11) {
        ...
        gen_helper_neon_addl_saturate_s64(tcg_res[pass], tcg_env, tcg_res[pass],
                                          tcg_passres);
      }
      else if (accop > 0) {
    ...
    for (pass = 0; pass < 2; pass++) {
      ...
      case 2: /* SSUBL, SSUBL2, USUBL, USUBL2 */
      {
        ...
        gen_neon_addl(size, (opcode == 2), tcg_passres, tcg_passres,
                      tcg_op2_64);
        break;
      }
      case 5: /* SABAL, SABAL2, UABAL, UABAL2 */
      ...
      case 13: /* SQDMULL, SQDMULL2 */
        assert(size == 1);
        gen_helper_neon_mull_s16(tcg_passres, tcg_op1, tcg_op2);
        gen_helper_neon_addl_saturate_s32(tcg_passres, tcg_env, tcg_passres,
                                          tcg_passres);
        break;
      default:
        g_assert_not_reached();
      ...
      if (accop != 0) {
        if (opcode == 9 || opcode == 11) {
          ...
          gen_neon_addl(size, (accop < 0), tcg_res[pass], tcg_res[pass],
                        tcg_passres);
        }
      }
    }
                            ...
                            int opcode, int rd, int rn, int rm)
{
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    gen_neon_addl(size, (opcode == 3), tcg_res[pass], tcg_op1, tcg_op2_wide);
  }
                                 ...
                                 int opcode, int rd, int rn, int rm)
{
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    static NeonGenNarrowFn *const narrowfns[3][2] = {
        {gen_helper_neon_narrow_high_u8, gen_helper_neon_narrow_round_high_u8},
...
static void disas_simd_three_reg_diff(DisasContext *s, uint32_t insn)
{
  ...
  case 14: /* PMULL, PMULL2 */
    ...
    case 0: /* PMULL.P8 */
      ...
      gen_gvec_op3_ool(s, true, rd, rn, rm, is_q, gen_helper_neon_pmull_h);
      break;
    ...
    case 3: /* PMULL.P64 */
      ...
      /* The Q field specifies lo/hi half input for this insn.  */
      gen_gvec_op3_ool(s, true, rd, rn, rm, is_q, gen_helper_gvec_pmull_q);
      break;
...
static void disas_simd_3same_float(DisasContext *s, uint32_t insn)
{
  ...
  case 0x7e: /* FMINP */
    ...
    handle_simd_3same_pair(s, is_q, 0, fpopcode, size ? MO_64 : MO_32, rn, rm,
                           rd);
    return;
  case 0x1b: /* FMULX */
  case 0x1f: /* FRECPS */
  ...
  case 0x79: /* FMLSL2 */
    ...
    if (fp_access_check(s)) {
      ...
      tcg_gen_gvec_3_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                         vec_full_reg_offset(s, rm), tcg_env, is_q ? 16 : 8,
...
static void disas_simd_three_reg_same_fp16(DisasContext *s, uint32_t insn)
{
  ...
  if (pairwise) {
    ...
    for (pass = 0; pass < maxpass; pass++) {
      ...
      switch (fpopcode) {
      case 0x10: /* FMAXNMP */
        gen_helper_advsimd_maxnumh(tcg_res[pass], tcg_op1, tcg_op2, fpst);
        break;
      case 0x12: /* FADDP */
      ...
      case 0x16: /* FMAXP */
        ...
        break;
      case 0x18: /* FMINNMP */
        gen_helper_advsimd_minnumh(tcg_res[pass], tcg_op1, tcg_op2, fpst);
        break;
      case 0x1e: /* FMINP */
    ...
    for (pass = 0; pass < elements; pass++) {
      ...
      case 0x1: /* FMLA */
        ...
        gen_helper_advsimd_muladdh(tcg_res, tcg_op1, tcg_op2, tcg_res, fpst);
        break;
      case 0x2: /* FADD */
        gen_helper_advsimd_addh(tcg_res, tcg_op1, tcg_op2, fpst);
        break;
...
static void disas_simd_three_reg_same_extra(DisasContext *s, uint32_t insn)
{
  ...
  case 0x04: /* SMMLA, UMMLA */
    gen_gvec_op4_ool(s, 1, rd, rn, rm, rd, 0,
                     u ? gen_helper_gvec_ummla_b : gen_helper_gvec_smmla_b);
    return;
  case 0x05: /* USMMLA */
  ...
  if (size == 2) {
    ...
    for (pass = 0; pass < maxpass; pass++) {
      ...
      if (accum) {
        ...
        if (size == 0) {
          gen_helper_neon_addl_u16(tcg_res[pass], tcg_res[pass], tcg_op);
        }
        else {
          gen_helper_neon_addl_u32(tcg_res[pass], tcg_res[pass], tcg_op);
        }
      }
    }
  }
...
static void disas_simd_two_reg_misc(DisasContext *s, uint32_t insn)
{
  ...
  if (size == 3) {
    ...
    for (pass = 0; pass < 2; pass++) {
      ...
      handle_2misc_64(s, opcode, u, tcg_res, tcg_op, tcg_rmode, tcg_fpstatus);
    ...
    for (pass = 0; pass < (is_q ? 4 : 2); pass++) {
      ...
      if (size == 2) {
        ...
        case 0x3a: /* FCVTPS */
        case 0x3b: /* FCVTZS */
          gen_helper_vfp_tosls(tcg_res, tcg_op, tcg_constant_i32(0),
                               tcg_fpstatus);
          break;
        case 0x5a: /* FCVTNU */
        case 0x5b: /* FCVTMU */
        ...
        case 0x7a: /* FCVTPU */
        case 0x7b: /* FCVTZU */
          gen_helper_vfp_touls(tcg_res, tcg_op, tcg_constant_i32(0),
                               tcg_fpstatus);
          break;
        case 0x18: /* FRINTN */
        case 0x19: /* FRINTM */
...
static void disas_simd_indexed(DisasContext *s, uint32_t insn)
{
  ...
  case 0x17: /* FCMLA #270 */
  {
    ...
    tcg_gen_gvec_4_ptr(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                       vec_full_reg_offset(s, rm), vec_full_reg_offset(s, rd),
                       fpst, is_q ? 16 : 8, vec_full_reg_size(s), data,
  ...
  case 0x08: /* MUL */
    if (!is_long && !is_scalar) {
      ...
      tcg_gen_gvec_3_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                         vec_full_reg_offset(s, rm), is_q ? 16 : 8,
                         vec_full_reg_size(s), index, fns[size - 1]);
      return;
    }
    break;
  ...
  case 0x10: /* MLA */
    if (!is_long && !is_scalar) {
      ...
      tcg_gen_gvec_4_ool(vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
                         vec_full_reg_offset(s, rm), vec_full_reg_offset(s, rd),
                         is_q ? 16 : 8, vec_full_reg_size(s), index,
                         fns[size - 1]);
      return;
    }
    break;
  ...
  if (size == 3) {
    ...
    for (pass = 0; pass < maxpasses; pass++) {
      ...
      case 0x01: /* FMLA */
        ...
        case 1:
          ...
          if (is_scalar) {
            ...
            gen_helper_advsimd_muladd2h(tcg_res, tcg_op, tcg_idx, tcg_res,
                                        fpst);
          }
          break;
        case 2:
      ...
      case 0x09: /* FMUL */
        ...
        case 1:
          if (is_scalar) {
            gen_helper_advsimd_mulh(tcg_res, tcg_op, tcg_idx, fpst);
          }
          else {
            gen_helper_advsimd_mul2h(tcg_res, tcg_op, tcg_idx, fpst);
          }
          break;
        case 2:
      ...
      case 0x19: /* FMULX */
        ...
        case 1:
          if (is_scalar) {
            ...
            gen_helper_advsimd_mulx2h(tcg_res, tcg_op, tcg_idx, fpst);
          }
          break;
        case 2:
      ...
      case 0x0c: /* SQDMULH */
        if (size == 1) {
          gen_helper_neon_qdmulh_s16(tcg_res, tcg_env, tcg_op, tcg_idx);
        }
        else {
          gen_helper_neon_qdmulh_s32(tcg_res, tcg_env, tcg_op, tcg_idx);
        }
        break;
      case 0x0d: /* SQRDMULH */
        if (size == 1) {
          gen_helper_neon_qrdmulh_s16(tcg_res, tcg_env, tcg_op, tcg_idx);
        }
        else {
          gen_helper_neon_qrdmulh_s32(tcg_res, tcg_env, tcg_op, tcg_idx);
        }
        break;
      case 0x1d: /* SQRDMLAH */
        read_vec_element_i32(s, tcg_res, rd, pass, is_scalar ? size : MO_32);
        if (size == 1) {
          gen_helper_neon_qrdmlah_s16(tcg_res, tcg_env, tcg_op, tcg_idx,
                                      tcg_res);
        }
        else {
          gen_helper_neon_qrdmlah_s32(tcg_res, tcg_env, tcg_op, tcg_idx,
                                      tcg_res);
        }
        break;
      case 0x1f: /* SQRDMLSH */
        read_vec_element_i32(s, tcg_res, rd, pass, is_scalar ? size : MO_32);
        if (size == 1) {
          gen_helper_neon_qrdmlsh_s16(tcg_res, tcg_env, tcg_op, tcg_idx,
                                      tcg_res);
        }
        else {
          gen_helper_neon_qrdmlsh_s32(tcg_res, tcg_env, tcg_op, tcg_idx,
                                      tcg_res);
        }
        break;
      default:
    ...
    if (size == 2) {
      ...
      for (pass = 0; pass < (is_scalar ? 1 : 2); pass++) {
        ...
        switch (opcode) {
        case 0x2: /* SMLAL, SMLAL2, UMLAL, UMLAL2 */
          gen_helper_neon_addl_u32(tcg_res[pass], tcg_res[pass], tcg_passres);
          break;
        case 0x6: /* SMLSL, SMLSL2, UMLSL, UMLSL2 */
          gen_helper_neon_subl_u32(tcg_res[pass], tcg_res[pass], tcg_passres);
          break;
        case 0x7: /* SQDMLSL, SQDMLSL2 */
          gen_helper_neon_negl_u32(tcg_passres, tcg_passres);
          /* fall through */
...
static void disas_crypto_xar(DisasContext *s, uint32_t insn)
{
  ...
  gen_gvec_xar(MO_64, vec_full_reg_offset(s, rd), vec_full_reg_offset(s, rn),
               vec_full_reg_offset(s, rm), imm6, 16, vec_full_reg_size(s));
}
...
static bool is_guarded_page(CPUARMState *env, DisasContext *s)
{
   ...
   */
  flags = probe_access_full(env, addr, 0, MMU_INST_FETCH, mmu_idx, false, &host,
                            &full, 0);
