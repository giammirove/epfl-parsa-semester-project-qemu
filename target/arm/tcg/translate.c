...
void arm_translate_init(void)
{
  ...
  for (i = 0; i < 16; i++) {
    cpu_R[i] = tcg_global_mem_new_i32(tcg_env, offsetof(CPUARMState, regs[i]),
                                      regnames[i]);
  }
  ...
  cpu_exclusive_addr = tcg_global_mem_new_i64(
      tcg_env, offsetof(CPUARMState, exclusive_addr), "exclusive_addr");
  cpu_exclusive_val = tcg_global_mem_new_i64(
      tcg_env, offsetof(CPUARMState, exclusive_val), "exclusive_val");
                                     ...
                                     int flags)
{
  if (flags) {
    switch (shiftop) {
    case 0:
      gen_helper_shl_cc(var, tcg_env, var, shift);
      break;
    case 1:
      gen_helper_shr_cc(var, tcg_env, var, shift);
      break;
    case 2:
      gen_helper_sar_cc(var, tcg_env, var, shift);
      break;
    case 3:
      gen_helper_ror_cc(var, tcg_env, var, shift);
      break;
    }
  }
  else {
...
{
  static const MemOp mop_align[] = {0,          MO_ALIGN_2,  MO_ALIGN_4,
                                    MO_ALIGN_8, MO_ALIGN_16, MO_ALIGN_32};
...
static void gen_msr_banked(DisasContext *s, int r, int sysm, int rn)
{
  ...
  gen_helper_msr_banked(tcg_env, tcg_reg, tcg_constant_i32(tgtmode),
...
static void gen_mrs_banked(DisasContext *s, int r, int sysm, int rn)
{
  ...
  gen_helper_mrs_banked(tcg_reg, tcg_env, tcg_constant_i32(tgtmode),
                        tcg_constant_i32(regno));
...
{
  static gen_helper_gvec_3_ptr *const fns[2] = {gen_helper_gvec_qrdmlah_s16,
...
void gen_gvec_sqrdmlsh_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs,
                          uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static gen_helper_gvec_3_ptr *const fns[2] = {gen_helper_gvec_qrdmlsh_s16,
...
#define GEN_CMP0(NAME, COND)                                                   \
  void NAME(unsigned vece, uint32_t d, uint32_t m, uint32_t opr_sz,            \
            uint32_t max_sz)                                                   \
  {                                                                            \
...
void gen_gvec_srshr(unsigned vece, uint32_t rd_ofs, uint32_t rm_ofs,
                    int64_t shift, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_shri_vec, INDEX_op_sari_vec,
                                         INDEX_op_add_vec, 0};
  static const GVecGen2i ops[4] = {
      {.fni8 = gen_srshr8_i64,
       .fniv = gen_srshr_vec,
...
void gen_gvec_uqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs,
                       uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_usadd_vec, INDEX_op_cmp_vec,
                                         INDEX_op_add_vec, 0};
  ...
  tcg_gen_gvec_4(rd_ofs, offsetof(CPUARMState, vfp.qc), rn_ofs, rm_ofs, opr_sz,
                 max_sz, &ops[vece]);
}
...
void gen_gvec_sqadd_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs,
                       uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_ssadd_vec, INDEX_op_cmp_vec,
                                         INDEX_op_add_vec, 0};
  static const GVecGen4 ops[4] = {
      {.fniv = gen_sqadd_vec,
...
void gen_gvec_uqsub_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs,
                       uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_ussub_vec, INDEX_op_cmp_vec,
                                         INDEX_op_sub_vec, 0};
  static const GVecGen4 ops[4] = {
      {.fniv = gen_uqsub_vec,
...
void gen_gvec_sqsub_qc(unsigned vece, uint32_t rd_ofs, uint32_t rn_ofs,
                       uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_sssub_vec, INDEX_op_cmp_vec,
                                         INDEX_op_sub_vec, 0};
  static const GVecGen4 ops[4] = {
      {.fniv = gen_sqsub_vec,
                   ...
                   uint32_t rm_ofs, uint32_t opr_sz, uint32_t max_sz)
{
  static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_add_vec,
                                         INDEX_op_smin_vec, INDEX_op_smax_vec,
                                         0};
  static const GVecGen3 ops[4] = {
      {.fniv = gen_saba_vec,
...
{
  static const TCGOpcode vecop_list[] = {INDEX_op_sub_vec, INDEX_op_add_vec,
                                         INDEX_op_umin_vec, INDEX_op_umax_vec,
                                         0};
  static const GVecGen3 ops[4] = {
      {.fniv = gen_uaba_vec,
                           ...
                           int rt2)
{
  ...
  case 14:
    if (is64) {
      syndrome = syn_cp14_rrt_trap(1, 0xe, opc1, crm, rt, rt2, isread, false);
    }
    else {
      syndrome =
          syn_cp14_rt_trap(1, 0xe, opc1, opc2, crn, crm, rt, isread, false);
    }
    break;
  case 15:
    if (is64) {
      syndrome = syn_cp15_rrt_trap(1, 0xe, opc1, crm, rt, rt2, isread, false);
    }
    else {
      syndrome =
          syn_cp15_rt_trap(1, 0xe, opc1, opc2, crn, crm, rt, isread, false);
    }
    break;
  default:
    /*
...
static bool valid_cp(DisasContext *s, int cp)
{
  ...
  if (arm_dc_feature(s, ARM_FEATURE_V8) && !arm_dc_feature(s, ARM_FEATURE_M)) {
    return cp >= 14;
  }
  return cp < 8 || cp >= 14;
}
...
static bool trans_MRC(DisasContext *s, arg_MRC *a)
{
  ...
  do_coproc_insn(s, a->cp, false, a->opc1, a->crn, a->crm, a->opc2, true, a->rt,
                 0);
  return true;
}
                          ...
                          WideShiftImmFn *fn)
{
  ...
  if (!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) ||
      a->rdahi == 13) {
...
static bool do_mve_shl_rr(DisasContext *s, arg_mve_shl_rr *a, WideShiftFn *fn)
{
  ...
  if (!dc_isar_feature(aa32_mve, s) || !arm_dc_feature(s, ARM_FEATURE_M_MAIN) ||
      a->rdahi == 13 || a->rm == 13 || a->rm == 15 || a->rm == a->rdahi ||
      a->rm == a->rdalo) {
...
static bool op_qaddsub(DisasContext *s, arg_rrr *a, bool add, bool doub)
{
  ...
  if (s->thumb ? !arm_dc_feature(s, ARM_FEATURE_THUMB_DSP) : !ENABLE_ARCH_5TE) {
    return false;
  }
...
{
  if (!arm_dc_feature(s, ARM_FEATURE_M) || !arm_dc_feature(s, ARM_FEATURE_V8)) {
    return false;
  }
  /*
   * SG (v8M only)
...
static bool trans_PLI(DisasContext *s, arg_PLD *a) { return ENABLE_ARCH_7; }
...
static void disas_thumb2_insn(DisasContext *s, uint32_t insn)
{
   ...
   */
  if (arm_dc_feature(s, ARM_FEATURE_M) && !arm_dc_feature(s, ARM_FEATURE_V7)) {
    int i;
    bool found = false;
    static const uint32_t armv6m_insn[] = {
        0xf3808000 /* msr */, 0xf3b08040 /* dsb */, 0xf3b08050 /* dmb */,
        0xf3b08060 /* isb */, 0xf3e08000 /* mrs */, 0xf000d000 /* bl */};
    static const uint32_t armv6m_mask[] = {0xffe0d000, 0xfff0d0f0, 0xfff0d0f0,
...
static void arm_tr_init_disas_context(DisasContextBase *dcbase, CPUState *cs)
{
  ...
  if (arm_feature(env, ARM_FEATURE_M)) {
    ...
    dc->v7m_new_fp_ctxt_needed = EX_TBFLAG_M32(tb_flags, NEW_FP_CTXT_NEEDED);
