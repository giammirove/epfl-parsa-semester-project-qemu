...
#define VIRTIO_NET_RSS_SUPPORTED_HASHES                                        \
  (VIRTIO_NET_RSS_HASH_TYPE_IPv4 | VIRTIO_NET_RSS_HASH_TYPE_TCPv4 |            \
   VIRTIO_NET_RSS_HASH_TYPE_UDPv4 | VIRTIO_NET_RSS_HASH_TYPE_IPv6 |            \
   VIRTIO_NET_RSS_HASH_TYPE_TCPv6 | VIRTIO_NET_RSS_HASH_TYPE_UDPv6 |           \
   VIRTIO_NET_RSS_HASH_TYPE_IP_EX | VIRTIO_NET_RSS_HASH_TYPE_TCP_EX |          \
...
static int vq2q(int queue_index) { return queue_index / 2; }
...
static void virtio_net_get_config(VirtIODevice *vdev, uint8_t *config)
{
  ...
  if (nc->peer && nc->peer->info->type == NET_CLIENT_DRIVER_VHOST_VDPA) {
    ...
    netcfg.status |= virtio_tswap16(vdev, n->status & VIRTIO_NET_S_ANNOUNCE);
    memcpy(config, &netcfg, n->config_size);
  }
}
...
static void virtio_net_set_config(VirtIODevice *vdev, const uint8_t *config)
{
  ...
  if (nc->peer && nc->peer->info->type == NET_CLIENT_DRIVER_VHOST_VDPA) {
    vhost_net_set_config(get_vhost_net(nc->peer), (uint8_t *)&netcfg, 0,
                         n->config_size, VHOST_SET_CONFIG_TYPE_FRONTEND);
  }
}
...
static int virtio_net_set_vnet_endian_one(VirtIODevice *vdev,
                                          NetClientState *peer, bool enable)
{
...
static void virtio_net_vnet_endian_status(VirtIONet *n, uint8_t status)
{
  ...
  if (virtio_net_started(n, status)) {
    ...
    n->needs_vnet_hdr_swap =
        virtio_net_set_vnet_endian(vdev, n->nic->ncs, queue_pairs, true);
  }
...
static void virtio_net_set_status(struct VirtIODevice *vdev, uint8_t status)
{
  ...
  for (i = 0; i < n->max_queue_pairs; i++) {
    ...
    queue_started = virtio_net_started(n, queue_status) && !n->vhost_started;
...
static int peer_has_vnet_hdr(VirtIONet *n) { return n->has_vnet_hdr; }
                                       ...
                                       int version_1, int hash_report)
{
   ...
   */
  if (version_1) {
    n->guest_hdr_len = hash_report ? sizeof(struct virtio_net_hdr_v1_hash)
                                        ...
                                        Error **errp)
{
  ...
  if (n->mtu_bypass_backend && (n->host_features & 1ULL << VIRTIO_NET_F_MTU)) {
...
static uint64_t virtio_net_guest_offloads_by_features(uint64_t features)
{
  static const uint64_t guest_offloads_mask =
      ...
      (1ULL << VIRTIO_NET_F_GUEST_UFO) | (1ULL << VIRTIO_NET_F_GUEST_USO4) |
...
static int failover_set_primary(DeviceState *dev, void *opaque)
{
  ...
  PCIDevice *pci_dev =
      (PCIDevice *)object_dynamic_cast(OBJECT(dev), TYPE_PCI_DEVICE);
...
static DeviceState *failover_find_primary_device(VirtIONet *n)
{
  ...
  qbus_walk_children(sysbus_get_default(), failover_set_primary, NULL, NULL,
                     NULL, &fdev);
  return fdev.dev;
}
...
static void failover_add_primary(VirtIONet *n, Error **errp)
{
  ...
  dev = qdev_device_add_from_qdict(n->primary_opts, n->primary_opts_from_json,
                                   &err);
  if (err) {
...
static void virtio_net_set_features(VirtIODevice *vdev, uint64_t features)
{
  ...
  virtio_net_set_mrg_rx_bufs(
      n, virtio_has_feature(features, VIRTIO_NET_F_MRG_RXBUF),
      virtio_has_feature(features, VIRTIO_F_VERSION_1),
      virtio_has_feature(features, VIRTIO_NET_F_HASH_REPORT));
...
static int virtio_net_handle_mac(VirtIONet *n, uint8_t cmd, struct iovec *iov,
                                 unsigned int iov_cnt)
{
  ...
  if (mac_data.entries <= MAC_TABLE_ENTRIES) {
    s = iov_to_buf(iov, iov_cnt, 0, macs, mac_data.entries * ETH_ALEN);
...
static bool virtio_net_attach_epbf_rss(VirtIONet *n)
{
  ...
  if (!ebpf_rss_set_all(&n->ebpf_rss, &config, n->rss_data.indirections_table,
                        n->rss_data.key)) {
    return false;
  }
...
static bool virtio_net_load_ebpf(VirtIONet *n, Error **errp)
{
  ...
  if (virtio_net_attach_ebpf_to_backend(n->nic, -1)) {
    if (!(n->ebpf_rss_fds && virtio_net_load_ebpf_fds(n, errp))) {
...
static uint16_t virtio_net_handle_rss(VirtIONet *n, struct iovec *iov,
                                      unsigned int iov_cnt, bool do_rss)
{
  VirtIODevice *vdev = VIRTIO_DEVICE(n);
  ...
  n->rss_data.default_queue =
      do_rss ? virtio_lduw_p(vdev, &cfg.unclassified_queue) : 0;
  ...
  s = iov_to_buf(iov, iov_cnt, offset, n->rss_data.indirections_table,
                 size_get);
  if (s != size_get) {
  ...
  trace_virtio_net_rss_enable(n->rss_data.hash_types,
                              n->rss_data.indirections_len, temp.b);
  return queue_pairs;
error:
...
static int virtio_net_handle_mq(VirtIONet *n, uint8_t cmd, struct iovec *iov,
                                unsigned int iov_cnt)
{
  VirtIODevice *vdev = VIRTIO_DEVICE(n);
...
size_t virtio_net_handle_ctrl_iov(VirtIODevice *vdev, const struct iovec *in_sg,
                                  unsigned in_num, const struct iovec *out_sg,
                                  unsigned out_num)
{
  VirtIONet *n = VIRTIO_NET(vdev);
...
static int virtio_net_has_buffers(VirtIONetQueue *q, int bufsize)
{
  ...
  if (virtio_queue_empty(q->rx_vq) ||
      (n->mergeable_rx_bufs && !virtqueue_avail_bytes(q->rx_vq, bufsize, 0))) {
...
static uint8_t virtio_net_get_hash_type(bool hasip4, bool hasip6,
                                  ...
                                  size_t size)
{
  ...
  static const uint8_t reports[NetPktRssIpV6UdpEx + 1] = {
      VIRTIO_NET_HASH_REPORT_IPv4,    VIRTIO_NET_HASH_REPORT_TCPv4,
      VIRTIO_NET_HASH_REPORT_TCPv6,   VIRTIO_NET_HASH_REPORT_IPv6,
      VIRTIO_NET_HASH_REPORT_IPv6_EX, VIRTIO_NET_HASH_REPORT_TCPv6_EX,
      VIRTIO_NET_HASH_REPORT_UDPv4,   VIRTIO_NET_HASH_REPORT_UDPv6,
                                      ...
                                      size_t size, bool no_rss)
{
  ...
  while (offset < size) {
    ...
    if (!elem) {
      if (i) {
        virtio_error(vdev,
                     ...
                     i, n->mergeable_rx_bufs, offset, size, n->guest_hdr_len,
                     n->host_hdr_len, vdev->guest_features);
      }
      err = -1;
      goto err;
    }
    ...
    if (elem->in_num < 1) {
      virtio_error(vdev, "virtio-net receive queue contains no in buffers");
    ...
    if (i == 0) {
      ...
      if (n->mergeable_rx_bufs) {
        mhdr_cnt = iov_copy(mhdr_sg, ARRAY_SIZE(mhdr_sg), sg, elem->in_num,
      ...
      if (n->rss_data.populate_hash) {
        offset = sizeof(mhdr);
        iov_from_buf(sg, elem->in_num, offset, buf + offset,
                     n->host_hdr_len - sizeof(mhdr));
      }
    ...
    if (vdev->lm_logging_ctrl == LM_ENABLE) {
      ...
      uint8_t val = 0;
      address_space_read_cached(&caches->bitmap, index, &val, sizeof(val));
      val |= 1 << shift;
      address_space_write_cached(&caches->bitmap, index, &val, sizeof(val));
  ...
  if (mhdr_cnt) {
    ...
    iov_from_buf(mhdr_sg, mhdr_cnt, 0, &mhdr.num_buffers,
                 sizeof mhdr.num_buffers);
  }
                                         ...
                                         VirtioNetRscUnit *unit)
{
  ...
  unit->tcp =
      (struct tcp_header *)(((uint8_t *)unit->ip) + sizeof(struct ip6_header));
  unit->tcp_hdrlen = (htons(unit->tcp->th_offset_flags) & 0xF000) >> 10;
                                            ...
                                            VirtioNetRscUnit *n_unit)
{
  ...
  if (nseq == oseq) {
    if ((o_unit->payload == 0) && n_unit->payload) {
      ...
      return virtio_net_rsc_handle_ack(chain, seg, buf, n_unit->tcp,
                                       o_unit->tcp);
    }
  }
...
static size_t virtio_net_rsc_do_coalesce(VirtioNetRscChain *chain,
                                         NetClientState *nc, const uint8_t *buf,
                                         size_t size, VirtioNetRscUnit *unit)
{
  int ret;
...
static size_t virtio_net_rsc_drain_flow(VirtioNetRscChain *chain,
                                        NetClientState *nc, const uint8_t *buf,
                                        size_t size, uint16_t ip_start,
                                        uint16_t ip_size, uint16_t tcp_port)
{
  VirtioNetRscSeg *seg, *nseg;
                                      ...
                                      size_t size)
{
  ...
  if (ret == RSC_BYPASS) {
    ...
    return virtio_net_rsc_drain_flow(
        chain, nc, buf, size, ((hdr_len + sizeof(struct eth_header)) + 12),
                                      ...
                                      const uint8_t *buf, size_t size)
{
  ...
  if (RSC_CANDIDATE !=
      virtio_net_rsc_sanity_check6(chain, unit.ip, buf, size)) {
    return virtio_net_do_receive(nc, buf, size);
  }
  ...
  if (ret == RSC_BYPASS) {
    ...
    return virtio_net_rsc_drain_flow(
        chain, nc, buf, size, ((hdr_len + sizeof(struct eth_header)) + 8),
...
static VirtioNetRscChain *
virtio_net_rsc_lookup_chain(VirtIONet *n, NetClientState *nc, uint16_t proto)
{
  VirtioNetRscChain *chain;
  ...
  chain->drain_timer =
      timer_new_ns(QEMU_CLOCK_HOST, virtio_net_rsc_purge, chain);
...
static ssize_t virtio_net_rsc_receive(NetClientState *nc, const uint8_t *buf,
                                      size_t size)
{
  uint16_t proto;
  VirtioNetRscChain *chain;
...
static int32_t virtio_net_flush_tx(VirtIONetQueue *q)
{
  ...
  for (;;) {
    ...
    if (n->host_hdr_len != n->guest_hdr_len) {
      unsigned sg_num =
          iov_copy(sg, ARRAY_SIZE(sg), out_sg, out_num, 0, n->host_hdr_len);
      sg_num += iov_copy(sg + sg_num, ARRAY_SIZE(sg) - sg_num, out_sg, out_num,
                         n->guest_hdr_len, -1);
      out_num = sg_num;
      out_sg = sg;
    }
...
static void virtio_net_add_queue(VirtIONet *n, int index)
{
  ...
  n->vqs[index].rx_vq =
      virtio_add_queue(vdev, n->net_conf.rx_queue_size, virtio_net_handle_rx);

  if (n->net_conf.tx && !strcmp(n->net_conf.tx, "timer")) {
    n->vqs[index].tx_vq = virtio_add_queue(vdev, n->net_conf.tx_queue_size,
                                           virtio_net_handle_tx_timer);
    n->vqs[index].tx_timer =
        timer_new_ns(QEMU_CLOCK_VIRTUAL, virtio_net_tx_timer, &n->vqs[index]);
  }
  else {
    n->vqs[index].tx_vq = virtio_add_queue(vdev, n->net_conf.tx_queue_size,
                                           virtio_net_handle_tx_bh);
    n->vqs[index].tx_bh = qemu_bh_new_guarded(
        virtio_net_tx_bh, &n->vqs[index], &DEVICE(vdev)->mem_reentrancy_guard);
  }
...
static void virtio_net_change_num_queue_pairs(VirtIONet *n,
...
static int virtio_net_post_load_device(void *opaque, int version_id)
{
  ...
  virtio_net_set_mrg_rx_bufs(
      n, n->mergeable_rx_bufs,
      virtio_vdev_has_feature(vdev, VIRTIO_F_VERSION_1),
      virtio_vdev_has_feature(vdev, VIRTIO_NET_F_HASH_REPORT));
  ...
  if (virtio_vdev_has_feature(vdev, VIRTIO_NET_F_GUEST_ANNOUNCE) &&
    ...
    qemu_announce_timer_reset(&n->announce_timer, migrate_announce_params(),
                              QEMU_CLOCK_VIRTUAL, virtio_net_announce_timer, n);
    if (n->announce_timer.round) {
...
static const VMStateDescription vmstate_virtio_net_queue_tx_waiting = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT32(tx_waiting, VirtIONetQueue),
                                     VMSTATE_END_OF_LIST()},
};
...
static const VMStateDescription vmstate_virtio_net_tx_waiting = {
    ...
    .fields =
        (const VMStateField[]){
            VMSTATE_STRUCT_VARRAY_POINTER_UINT16(
                vqs_1, struct VirtIONetMigTmp, curr_queue_pairs_1,
                vmstate_virtio_net_queue_tx_waiting, struct VirtIONetQueue),
            VMSTATE_END_OF_LIST()},
};
...
static const VMStateDescription vmstate_virtio_net_has_ufo = {
    ...
    .fields =
        (const VMStateField[]){VMSTATE_UINT8(has_ufo, struct VirtIONetMigTmp),
                               VMSTATE_END_OF_LIST()},
};
...
static const VMStateDescription vmstate_virtio_net_has_vnet = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT32(has_vnet_hdr,
                                                    struct VirtIONetMigTmp),
                                     VMSTATE_END_OF_LIST()},
};
...
static const VMStateDescription vmstate_virtio_net_device = {
    ...
    .fields =
        (const VMStateField[]){
             ...
             */
            VMSTATE_VBUFFER_MULTIPLY(mac_table.macs, VirtIONet, 0,
                                     mac_table_fits, mac_table.in_use,
                                     ETH_ALEN),
             ...
             * buffer; hold onto your endiannesses; it's actually used as a
             * bitmap but based on the uint.
             */
            ...
            VMSTATE_UINT8(alluni, VirtIONet), VMSTATE_UINT8(nomulti, VirtIONet),
            VMSTATE_UINT8(nouni, VirtIONet), VMSTATE_UINT8(nobcast, VirtIONet),
            VMSTATE_WITH_TMP(VirtIONet, struct VirtIONetMigTmp,
                             vmstate_virtio_net_has_ufo),
            VMSTATE_SINGLE_TEST(max_queue_pairs, VirtIONet,
                                max_queue_pairs_gt_1, 0,
                                vmstate_info_uint16_equal, uint16_t),
            VMSTATE_UINT16_TEST(curr_queue_pairs, VirtIONet,
            ...
            VMSTATE_END_OF_LIST()},
    .subsections =
        (const VMStateDescription *const[]){&vmstate_virtio_net_rss, NULL}};
...
static bool virtio_net_guest_notifier_pending(VirtIODevice *vdev, int idx)
{
  ...
  if (!virtio_vdev_has_feature(vdev, VIRTIO_NET_F_MQ) && idx == 2) {
    ...
    if (!virtio_vdev_has_feature(vdev, VIRTIO_NET_F_CTRL_VQ)) {
      qemu_log_mask(LOG_GUEST_ERROR, "%s: bogus vq index ignored\n", __func__);
      return false;
    }
...
static void virtio_net_device_realize(DeviceState *dev, Error **errp)
{
  ...
  n->net_conf.tx_queue_size =
      MIN(virtio_net_max_tx_queue_size(n), n->net_conf.tx_queue_size);
  ...
  if (n->netclient_type) {
     ...
     */
    n->nic = qemu_new_nic(&net_virtio_info, &n->nic_conf, n->netclient_type,
                          n->netclient_name, &dev->mem_reentrancy_guard, n);
  }
  else {
...
static const VMStateDescription vmstate_virtio_net = {
    ...
    .version_id = VIRTIO_NET_VM_VERSION,
    .fields =
        (const VMStateField[]){VMSTATE_VIRTIO_DEVICE, VMSTATE_END_OF_LIST()},
...
static Property virtio_net_properties[] = {
    ...
    DEFINE_PROP_BIT64("status", VirtIONet, host_features, VIRTIO_NET_F_STATUS,
                      true),
    DEFINE_PROP_BIT64("ctrl_vq", VirtIONet, host_features, VIRTIO_NET_F_CTRL_VQ,
                      true),
    DEFINE_PROP_BIT64("ctrl_rx", VirtIONet, host_features, VIRTIO_NET_F_CTRL_RX,
                      true),
    ...
    DEFINE_PROP_BIT64("rss", VirtIONet, host_features, VIRTIO_NET_F_RSS, false),
    DEFINE_PROP_BIT64("hash", VirtIONet, host_features,
                      VIRTIO_NET_F_HASH_REPORT, false),
    DEFINE_PROP_ARRAY("ebpf-rss-fds", VirtIONet, nr_ebpf_rss_fds, ebpf_rss_fds,
