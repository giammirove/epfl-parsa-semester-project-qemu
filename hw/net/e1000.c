...
DECLARE_OBJ_CHECKERS(E1000State, E1000BaseClass, E1000, TYPE_E1000_BASE)
...
static void e1000_autoneg_done(E1000State *s)
{
...
static void set_phy_ctrl(E1000State *s, int index, uint16_t val)
{
  ...
  s->phy_reg[MII_BMCR] = val & ~(0x3f | MII_BMCR_RESET | MII_BMCR_ANRESTART);
...
static const uint16_t phy_reg_init[] = {
    [MII_BMCR] = MII_BMCR_SPEED1000 | MII_BMCR_FD | MII_BMCR_AUTOEN,

    [MII_BMSR] = MII_BMSR_EXTCAP | MII_BMSR_LINK_ST | /* link initially up */
                 MII_BMSR_AUTONEG |
                 /* MII_BMSR_AN_COMP: initially NOT completed */
                 MII_BMSR_MFPS | MII_BMSR_EXTSTAT | MII_BMSR_10T_HD |
                 MII_BMSR_10T_FD | MII_BMSR_100TX_HD | MII_BMSR_100TX_FD,

    [MII_PHYID1] = 0x141,
    /* [MII_PHYID2] configured per DevId, from e1000_reset() */
    [MII_ANAR] = MII_ANAR_CSMACD | MII_ANAR_10 | MII_ANAR_10FD | MII_ANAR_TX |
                 MII_ANAR_TXFD | MII_ANAR_PAUSE | MII_ANAR_PAUSE_ASYM,
    [MII_ANLPAR] =
        MII_ANLPAR_10 | MII_ANLPAR_10FD | MII_ANLPAR_TX | MII_ANLPAR_TXFD,
    [MII_CTRL1000] =
        MII_CTRL1000_FULL | MII_CTRL1000_PORT | MII_CTRL1000_MASTER,
    [MII_STAT1000] = MII_STAT1000_HALF | MII_STAT1000_FULL | MII_STAT1000_ROK |
                     MII_STAT1000_LOK,
...
static const uint32_t mac_reg_init[] = {
    [PBA] = 0x00100030,
    [LEDCTL] = 0x602,
    [CTRL] = E1000_CTRL_SWDPIN2 | E1000_CTRL_SWDPIN0 | E1000_CTRL_SPD_1000 |
             E1000_CTRL_SLU,
    [STATUS] = 0x80000000 | E1000_STATUS_GIO_MASTER_ENABLE | E1000_STATUS_ASDV |
               E1000_STATUS_MTXCKOK | E1000_STATUS_SPEED_1000 |
               E1000_STATUS_FD | E1000_STATUS_LU,
    [MANC] = E1000_MANC_EN_MNG2HOST | E1000_MANC_RCV_TCO_EN |
             E1000_MANC_ARP_EN | E1000_MANC_0298_EN | E1000_MANC_RMCP_EN,
};
...
static void set_interrupt_cause(E1000State *s, int index, uint32_t val)
{
  ...
  if (!s->mit_irq_level && pending_ints) {
    ...
    timer_mod(s->mit_timer,
              qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + mit_delay * 256);
    s->mit_ide = 0;
  }
...
static void e1000_flush_queue_timer(void *opaque)
{
  E1000State *s = opaque;
...
static void set_rx_control(E1000State *s, int index, uint32_t val)
{
  ...
  DBGOUT(RX, "RCTL: %d, mac_reg[RCTL] = 0x%x\n", s->mac_reg[RDT],
         s->mac_reg[RCTL]);
  timer_mod(s->flush_queue_timer, qemu_clock_get_ms(QEMU_CLOCK_VIRTUAL) + 1000);
}
...
static void set_eecd(E1000State *s, int index, uint32_t val)
{
  ...
  s->eecd_state.old_eecd =
      val & (E1000_EECD_SK | E1000_EECD_CS | E1000_EECD_DI |
             E1000_EECD_FWE_MASK | E1000_EECD_REQ);
  ...
  if (++s->eecd_state.bitnum_in == 9 && !s->eecd_state.reading) {
    ...
    s->eecd_state.reading =
        (((s->eecd_state.val_in >> 6) & 7) == EEPROM_READ_OPCODE_MICROWIRE);
  }
...
static void putsum(uint8_t *data, uint32_t n, uint32_t sloc, uint32_t css,
                   uint32_t cse)
{
  uint32_t sum;
...
static inline void inc_tx_bcast_or_mcast_count(E1000State *s,
...
{
  static const int PTCregs[6] = {PTC64,  PTC127,  PTC255,
...
static void xmit_seg(E1000State *s)
{
  ...
  if (tp->cptse) {
    ...
    if (props->ip) { /* IPv4 */
      ...
      stw_be_p(tp->data + css + 4, lduw_be_p(tp->data + css + 4) + frames);
    }
    else { /* IPv6 */
    ...
    if (props->tcp) {
      ...
      stl_be_p(tp->data + css + 4,
               ldl_be_p(tp->data + css + 4) + sofar); /* seq */
...
static void process_tx_desc(E1000State *s, struct e1000_tx_desc *dp)
{
  ...
  if (e1000x_vlan_enabled(s->mac_reg) && e1000x_is_vlan_txd(txd_lower) &&
  ...
  if (tp->cptse) {
    ...
    do {
      ...
      sz = tp->size + bytes;
      if (sz >= tp->tso_props.hdr_len && tp->size < tp->tso_props.hdr_len) {
...
static uint32_t txdesc_writeback(E1000State *s, dma_addr_t base,
                                 struct e1000_tx_desc *dp)
{
  ...
  pci_dma_write(d, base + ((char *)&dp->upper - (char *)dp), &dp->upper,
                sizeof(dp->upper));
...
static int receive_filter(E1000State *s, const void *buf)
{
...
static bool e1000_has_rxbufs(E1000State *s, size_t total_size)
{
  ...
  if (s->mac_reg[RDH] < s->mac_reg[RDT]) {
    ...
    bufs = s->mac_reg[RDLEN] / sizeof(struct e1000_rx_desc) + s->mac_reg[RDT] -
           s->mac_reg[RDH];
  }
  else {
    return false;
...
static void e1000_receiver_overrun(E1000State *s, size_t size)
{
...
static ssize_t e1000_receive_iov(NetClientState *nc, const struct iovec *iov,
                                 int iovcnt)
{
  E1000State *s = qemu_get_nic_opaque(nc);
  PCIDevice *d = PCI_DEVICE(s);
...
#define LOW_BITS_SET_FUNC(num)                                                 \
  static void set_##num##bit(E1000State *s, int index, uint32_t val)           \
  {                                                                            \
...
static const readops macreg_readops[] = {
    ...
    [GORCH] = mac_read_clr8,
    [PRC64] = mac_read_clr4,
    [PRC127] = mac_read_clr4,
    [PRC255] = mac_read_clr4,
    [PRC511] = mac_read_clr4,
    [PRC1023] = mac_read_clr4,
    [PRC1522] = mac_read_clr4,
    [PTC64] = mac_read_clr4,
    [PTC127] = mac_read_clr4,
    [PTC255] = mac_read_clr4,
    [PTC511] = mac_read_clr4,
    [PTC1023] = mac_read_clr4,
    [PTC1522] = mac_read_clr4,
    [GPRC] = mac_read_clr4,
    [GPTC] = mac_read_clr4,
    [TPT] = mac_read_clr4,
    [TPR] = mac_read_clr4,
...
static const writeops macreg_writeops[] = {
    ...
    [IP6AT... IP6AT + 3] = &mac_writereg,
    [IP4AT... IP4AT + 6] = &mac_writereg,
...
static const uint8_t mac_reg_access[0x8000] = {
    ...
    [SCC] = markflag(MAC),
    [FCRUC] = markflag(MAC),
    [LATECOL] = markflag(MAC),
    [COLC] = markflag(MAC),
    [SEQEC] = markflag(MAC),
    [CEXTERR] = markflag(MAC),
    [XONTXC] = markflag(MAC),
    [XOFFRXC] = markflag(MAC),
    [RJC] = markflag(MAC),
    [RNBC] = markflag(MAC),
    [MGTPDC] = markflag(MAC),
    [MGTPTC] = markflag(MAC),
    ...
    [PRC255] = markflag(MAC),
    [PRC511] = markflag(MAC),
    [PRC1023] = markflag(MAC),
    [PRC1522] = markflag(MAC),
    [PTC64] = markflag(MAC),
    [PTC127] = markflag(MAC),
    [PTC255] = markflag(MAC),
    [PTC511] = markflag(MAC),
    [PTC1023] = markflag(MAC),
    [PTC1522] = markflag(MAC),
    [MPTC] = markflag(MAC),
    [BPTC] = markflag(MAC),
...
static void e1000_io_write(void *opaque, hwaddr addr, uint64_t val,
                           unsigned size)
{
  E1000State *s = opaque;
...
static const VMStateDescription vmstate_e1000_mit_state = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT32(mac_reg[RDTR], E1000State),
...
static const VMStateDescription vmstate_e1000_tx_tso_state = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_INT8(tx.tso_props.tcp, E1000State), VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_e1000 = {
    ...
    .fields =
        (const VMStateField[]){
            ...
            VMSTATE_UINT32_SUB_ARRAY(mac_reg, E1000State, MTA,
                                     E1000_MC_TBL_SIZE),
            VMSTATE_UINT32_SUB_ARRAY(mac_reg, E1000State, VFTA,
                                     E1000_VLAN_FILTER_TBL_SIZE),
            VMSTATE_END_OF_LIST()},
    .subsections = (const VMStateDescription *const[]){
        &vmstate_e1000_mit_state, &vmstate_e1000_full_mac_state,
        &vmstate_e1000_tx_tso_state, NULL}};
...
static void e1000_mmio_setup(E1000State *d)
{
  printf("SETUP E1000\n");
  int i;
  const uint32_t excluded_regs[] = {E1000_MDIC, E1000_ICR,   E1000_ICS,
                                    E1000_IMS,  E1000_IMC,   E1000_TCTL,
                                    E1000_TDT,  PNPMMIO_SIZE};

  memory_region_init_io(&d->mmio, OBJECT(d), &e1000_mmio_ops, d, "e1000-mmio",
                        PNPMMIO_SIZE);
...
static void pci_e1000_realize(PCIDevice *pci_dev, Error **errp)
{
  ...
  e1000x_core_prepare_eeprom(d->eeprom_data, e1000_eeprom_template,
                             sizeof(e1000_eeprom_template),
                             PCI_DEVICE_GET_CLASS(pci_dev)->device_id, macaddr);

  d->nic =
      qemu_new_nic(&net_e1000_info, &d->conf, object_get_typename(OBJECT(d)),
                   dev->id, &dev->mem_reentrancy_guard, d);
  ...
  d->flush_queue_timer =
      timer_new_ms(QEMU_CLOCK_VIRTUAL, e1000_flush_queue_timer, d);
}

static Property e1000_properties[] = {
    DEFINE_NIC_PROPERTIES(E1000State, conf),
    DEFINE_PROP_BIT("extra_mac_registers", E1000State, compat_flags,
                    E1000_FLAG_MAC_BIT, true),
    DEFINE_PROP_BIT("migrate_tso_props", E1000State, compat_flags,
                    E1000_FLAG_TSO_BIT, true),
    DEFINE_PROP_BIT("init-vet", E1000State, compat_flags, E1000_FLAG_VET_BIT,
                    true),
...
static void e1000_instance_init(Object *obj)
{
  ...
  device_add_bootindex_property(obj, &n->conf.bootindex, "bootindex",
