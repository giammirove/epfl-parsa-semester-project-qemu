...
static const VMStateDescription vmstate_virtio_pci_modern_queue_state = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_UINT32_ARRAY(used, VirtIOPCIQueue, 2), VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_pci = {
    ...
    .subsections = (const VMStateDescription *const[]){
        &vmstate_virtio_pci_modern_state_sub, NULL}};
                                       ...
                                       int n, bool assign)
{
  ...
  hwaddr modern_addr =
      virtio_pci_queue_mem_mult(proxy) * virtio_get_queue_index(vq);
  hwaddr legacy_addr = VIRTIO_PCI_QUEUE_NOTIFY;

  if (assign) {
    if (modern) {
      memory_region_add_eventfd(modern_mr, modern_addr, 0, false, n, notifier);
      if (modern_pio) {
        memory_region_add_eventfd(modern_notify_mr, 0, 2, true, n, notifier);
      }
    }
    if (legacy) {
      memory_region_add_eventfd(legacy_mr, legacy_addr, 2, true, n, notifier);
    }
  }
  else {
    if (modern) {
      memory_region_del_eventfd(modern_mr, modern_addr, 0, false, n, notifier);
      if (modern_pio) {
        memory_region_del_eventfd(modern_notify_mr, 0, 2, true, n, notifier);
      }
    }
    if (legacy) {
      memory_region_del_eventfd(legacy_mr, legacy_addr, 2, true, n, notifier);
    }
  }
  return 0;
}
...
static void virtio_ioport_write(void *opaque, uint32_t addr, uint32_t val)
{
  ...
  case VIRTIO_PCI_STATUS:
    ...
    if (val == (VIRTIO_CONFIG_S_ACKNOWLEDGE | VIRTIO_CONFIG_S_DRIVER)) {
      pci_default_write_config(
          &proxy->pci_dev, PCI_COMMAND,
          proxy->pci_dev.config[PCI_COMMAND] | PCI_COMMAND_MASTER, 1);
    }
    break;
  ...
  case VIRTIO_MSI_QUEUE_VECTOR:
    ...
    break;
  default:
    qemu_log_mask(LOG_GUEST_ERROR, "%s: unexpected address 0x%x value 0x%x\n",
                  __func__, addr, val);
    break;
  }
}
...
static uint64_t virtio_pci_config_read(void *opaque, hwaddr addr, unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
...
static void virtio_pci_config_write(void *opaque, hwaddr addr, uint64_t val,
                                    unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  uint32_t config = VIRTIO_PCI_CONFIG_SIZE(&proxy->pci_dev);
                                                 ...
                                                 hwaddr *off, int len)
{
  ...
  for (i = 0; i < ARRAY_SIZE(proxy->regs); ++i) {
    reg = &proxy->regs[i];
    if (*off >= reg->offset && *off + len <= reg->offset + reg->size) {
      *off -= reg->offset;
      return &reg->mr;
    }
  }
 ...
 */
static void virtio_address_space_write(VirtIOPCIProxy *proxy, hwaddr addr,
...
static void virtio_address_space_read(VirtIOPCIProxy *proxy, hwaddr addr,
                                      uint8_t *buf, int len)
{
  uint64_t val;
  MemoryRegion *mr;
...
static uint32_t virtio_read_config(PCIDevice *pci_dev, uint32_t address,
                                   int len)
{
  VirtIOPCIProxy *proxy = VIRTIO_PCI(pci_dev);
  struct virtio_pci_cfg_cap *cfg;
...
static int kvm_virtio_pci_irqfd_use(VirtIOPCIProxy *proxy, EventNotifier *n,
                                    unsigned int vector)
{
  VirtIOIRQFD *irqfd = &proxy->vector_irqfd[vector];
...
static void virtio_pci_one_vector_mask(VirtIOPCIProxy *proxy,
                                       unsigned int queue_no,
                                       unsigned int vector, EventNotifier *n)
{
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
  VirtioDeviceClass *k = VIRTIO_DEVICE_GET_CLASS(vdev);
...
static void virtio_pci_vector_poll(PCIDevice *dev, unsigned int vector_start,
                                         ...
                                         bool with_irqfd)
{
  ...
  if (assign) {
    ...
    virtio_pci_set_guest_notifier_fd_handler(vdev, vq, n, false, with_irqfd);
    event_notifier_cleanup(notifier);
  }

  if (!msix_enabled(&proxy->pci_dev) && vdev->use_guest_notifier_mask &&
                                  ...
                                  struct virtio_pci_cap *cap)
{
  ...
  offset =
      pci_add_capability(dev, PCI_CAP_ID_VNDR, 0, cap->cap_len, &error_abort);
...
static void lm_enable(VirtIODevice *vdev)
{
  ...
  len = address_space_cache_init(&new->bitmap, vdev->dma_as, addr, size, true);
  if (len < size) {
...
static uint64_t virtio_pci_lm_read(void *opaque, hwaddr addr, unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
  ...
  default:
    if (addr >= LM_VRING_STATE_OFFSET && addr <= offset_end) {
      qid = (addr - LM_VRING_STATE_OFFSET) / virtio_pci_queue_mem_mult(proxy);
...
static void virtio_pci_lm_write(void *opaque, hwaddr addr, uint64_t val,
                                unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
  ...
  case LM_LOGGING_CTRL:
    ...
    case LM_ENABLE:
      ...
      break;
    default:
      virtio_error(vdev, "Unsupport LM_LOGGING_CTRL value: %" PRIx64, val);
      break;
    };
...
static void virtio_pci_common_write(void *opaque, hwaddr addr, uint64_t val,
                                    unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
...
static uint64_t virtio_pci_notify_read(void *opaque, hwaddr addr, unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
...
static void virtio_pci_notify_write(void *opaque, hwaddr addr, uint64_t val,
                                    unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
...
static void virtio_pci_notify_write_pio(void *opaque, hwaddr addr, uint64_t val,
                                        unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
...
static uint64_t virtio_pci_isr_read(void *opaque, hwaddr addr, unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
  uint64_t val;
...
static void virtio_pci_isr_write(void *opaque, hwaddr addr, uint64_t val,
                                 unsigned size)
{
}

static uint64_t virtio_pci_device_read(void *opaque, hwaddr addr, unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
  uint64_t val;
...
static void virtio_pci_device_write(void *opaque, hwaddr addr, uint64_t val,
                                    unsigned size)
{
  VirtIOPCIProxy *proxy = opaque;
  VirtIODevice *vdev = virtio_bus_get_device(&proxy->bus);
                                           ...
                                           const char *vdev_name)
{
  ...
  memory_region_init_io(&proxy->common.mr, OBJECT(proxy), &common_ops, proxy,
                        name->str, proxy->common.size);

  g_string_printf(name, "virtio-pci-isr-%s", vdev_name);
  memory_region_init_io(&proxy->isr.mr, OBJECT(proxy), &isr_ops, proxy,
                        name->str, proxy->isr.size);

  g_string_printf(name, "virtio-pci-device-%s", vdev_name);
  memory_region_init_io(&proxy->device.mr, OBJECT(proxy), &device_ops, proxy,
                        name->str, proxy->device.size);

  g_string_printf(name, "virtio-pci-notify-%s", vdev_name);
  memory_region_init_io(&proxy->notify.mr, OBJECT(proxy), &notify_ops, proxy,
                        name->str, proxy->notify.size);

  g_string_printf(name, "virtio-pci-notify-pio-%s", vdev_name);
  memory_region_init_io(&proxy->notify_pio.mr, OBJECT(proxy), &notify_pio_ops,
                        proxy, name->str, proxy->notify_pio.size);
  if (proxy->flags & VIRTIO_PCI_FLAG_VDPA) {
    g_string_printf(name, "virtio-pci-lm-%s", vdev_name);
    memory_region_init_io(&proxy->lm.mr, OBJECT(proxy), &lm_ops, proxy,
                          name->str, proxy->lm.size);
  }
}
...
static void virtio_pci_modern_io_region_map(VirtIOPCIProxy *proxy,
                                            ...
                                            struct virtio_pci_cap *cap)
{
  virtio_pci_modern_region_map(proxy, region, cap, &proxy->io_bar,
...
static void virtio_pci_modern_io_region_unmap(VirtIOPCIProxy *proxy,
                                              VirtIOPCIRegion *region)
{
  memory_region_del_subregion(&proxy->io_bar, &region->mr);
}
...
static void virtio_pci_device_plugged(DeviceState *d, Error **errp)
{
  ...
  if (modern) {
    ...
    if (proxy->flags & VIRTIO_PCI_FLAG_VDPA) {
      memory_region_add_subregion(&proxy->modern_bar, proxy->lm.offset,
                                  &proxy->lm.mr);
    }
  ...
  if (proxy->nvectors) {
    ...
    if (err) {
      ...
      if (err != -ENOTSUP) {
        warn_report("unable to init msix vectors to %" PRIu32, proxy->nvectors);
      }
      proxy->nvectors = 0;
    }
  }
...
static void virtio_pci_realize(PCIDevice *pci_dev, Error **errp)
{
  ...
  if (pcie_port && pci_is_express(pci_dev)) {
    ...
    pos = pci_add_capability(pci_dev, PCI_CAP_ID_PM, 0, PCI_PM_SIZEOF, errp);
    if (pos < 0) {
      return;
    }
    ...
    if (proxy->flags & VIRTIO_PCI_FLAG_INIT_PM) {
      ...
      pci_set_word(pci_dev->wmask + pos + PCI_PM_CTRL, PCI_PM_CTRL_STATE_MASK);
    }
...
static Property virtio_pci_properties[] = {
    ...
    DEFINE_PROP_BIT("ats", VirtIOPCIProxy, flags, VIRTIO_PCI_FLAG_ATS_BIT,
                    false),
    ...
    DEFINE_PROP_BIT("aer", VirtIOPCIProxy, flags, VIRTIO_PCI_FLAG_AER_BIT,
                    false),
    DEFINE_PROP_BIT("vdpa", VirtIOPCIProxy, flags, VIRTIO_PCI_FLAG_VDPA_BIT,
                    false),
