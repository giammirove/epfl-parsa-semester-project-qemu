...
{
  plugin_register_cb(id, QEMU_PLUGIN_EV_QFLEX_STATE, state);
}
                                          ...
                                          void *udata)
{
  if (!tb->mem_only) {
    int index =
        flags == QEMU_PLUGIN_CB_R_REGS || flags == QEMU_PLUGIN_CB_RW_REGS
            ? PLUGIN_CB_REGULAR_R
            : PLUGIN_CB_REGULAR;

    plugin_register_dyn_cb__udata(&tb->cbs[index], cb, flags, udata);
  }
}
...
{
  if (!tb->mem_only) {
    plugin_register_inline_op_on_entry(&tb->cbs[PLUGIN_CB_INLINE], 0, op, entry,
                                       imm);
  }
}
                                            ...
                                            void *udata)
{
  if (!insn->mem_only) {
    ...
    plugin_register_dyn_cb__udata(&insn->cbs[PLUGIN_CB_INSN][index], cb, flags,
                                  udata);
  }
}
    ...
    enum qemu_plugin_cb_flags flags, void *udata, uint64_t (*test_fun)(void))
{
  if (!insn->mem_only) {
    ...
    plugin_register_dyn_cb__qflex(&insn->cbs[PLUGIN_CB_INSN][index], cb, flags,
                                  udata, test_fun);
  }
}
...
{
  plugin_register_vcpu_mem_cb(&insn->cbs[PLUGIN_CB_MEM][PLUGIN_CB_REGULAR], cb,
                              flags, rw, udata);
}
                                                  ...
                                                  uint64_t vaddr)
{
  ...
  if (!tlb_plugin_lookup(cpu, vaddr, mmu_idx, hwaddr_info.is_store,
                         &hwaddr_info)) {
...
int qemu_plugin_num_vcpus(void) { return plugin_num_vcpus(); }
...
{
  GArray *find_data =
      g_array_new(true, true, sizeof(qemu_plugin_reg_descriptor));
