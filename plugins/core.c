...
{
  bitmap_copy(cpu->plugin_state->event_mask, &data.host_ulong,
              QEMU_PLUGIN_EV_MAX);
...
static void do_plugin_register_cb(qemu_plugin_id_t id,
                                  enum qemu_plugin_event ev, void *func,
                                  void *udata)
{
  struct qemu_plugin_ctx *ctx;
...
void plugin_register_cb_udata(qemu_plugin_id_t id, enum qemu_plugin_event ev,
...
void qemu_plugin_vcpu_init_hook(CPUState *cpu)
{
  ...
  success =
      g_hash_table_insert(plugin.cpu_ht, &cpu->cpu_index, &cpu->cpu_index);
  g_assert(success);
...
void qemu_plugin_qflex_state_hook(CPUState *cpu)
{
  plugin_cb__qlex_state(cpu, QEMU_PLUGIN_EV_QFLEX_STATE);
}
...
void plugin_register_dyn_cb__udata(GArray **arr, qemu_plugin_vcpu_udata_cb_t cb,
                                   ...
                                   uint64_t (*test_fun)(void *))
{
  ...
  /* Note flags are discarded as unused. */
  dyn_cb->f.vcpu_udata = cb;
  dyn_cb->type = PLUGIN_CB_QFLEX;
  dyn_cb->test = 23;
  // dyn_cb->test_fun = test_fun;
}

void plugin_register_vcpu_mem_cb(GArray **arr, void *cb,
                                 enum qemu_plugin_cb_flags flags,
                                 enum qemu_plugin_mem_rw rw, void *udata)
{
  struct qemu_plugin_dyn_cb *dyn_cb;
 ...
 */
QEMU_DISABLE_CFI
void qemu_plugin_vcpu_syscall(CPUState *cpu, int64_t num, uint64_t a1,
...
void exec_inline_op(struct qemu_plugin_dyn_cb *cb, int cpu_index)
{
  ...
  size_t elem_size =
      g_array_get_element_size(cb->inline_insn.entry.score->data);
...
void qemu_plugin_vcpu_mem_cb(CPUState *cpu, uint64_t vaddr, MemOpIdx oi,
                             enum qemu_plugin_mem_rw rw)
{
  ...
  for (i = 0; i < arr->len; i++) {
    ...
    case PLUGIN_CB_REGULAR:
      cb->f.vcpu_mem(cpu->cpu_index, make_plugin_meminfo(oi, rw), vaddr,
                     cb->userp);
      break;
...
void qemu_plugin_atexit_cb(void) { plugin_cb__udata(QEMU_PLUGIN_EV_ATEXIT); }

void qemu_plugin_register_atexit_cb(qemu_plugin_id_t id,
                                    qemu_plugin_udata_cb_t cb, void *udata)
{
...
void qemu_plugin_user_exit(void)
{
  ...
  CPU_FOREACH(cpu) { qemu_plugin_disable_mem_helpers(cpu); }
  qemu_rec_mutex_unlock(&plugin.lock);
...
void qemu_plugin_user_prefork_lock(void) { qemu_rec_mutex_lock(&plugin.lock); }
...
int plugin_num_vcpus(void) { return plugin.num_vcpus; }
